...
def create_scripts(dest_dir):
    ...
    if sys.platform == "win32":
        simnibs_sitecustomize = os.path.join(
            SIMNIBSDIR, "_internal_resources", "sitecustomize.py"
        )
        env_sitecustomize = os.path.join(
            os.path.dirname(sys.executable), "Lib", "site-packages", "sitecustomize.py"
        )
...
def _write_unix_sh(python_cli, bash_cli, commands='"$@"'):
    ...
    os.chmod(
        bash_cli, os.stat(bash_cli).st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH
    )
...
def _write_windows_cmd(python_cli, bash_cli, gui=False, commands="%*"):
    ...
    if gui:
        python_interpreter = (
            f'start "Loading SimNIBS" "{os.path.join(executables_dir, "pythonw.exe")}"'
        )
    else:
        python_interpreter = f'"{os.path.join(executables_dir, "python.exe")}"'
    with open(bash_cli, "w") as f:
...
def _get_activate_bin():
    activate_bin = os.path.abspath(
        os.path.join(os.path.dirname(sys.executable), "..", "..", "Scripts", "activate")
    )
    if os.path.isfile(activate_bin):
        return activate_bin
    activate_bin = os.path.abspath(
        os.path.join(os.path.dirname(sys.executable), "Scripts", "activate.bat")
    )
    if os.path.isfile(activate_bin):
        return activate_bin
    else:
...
def setup_gmsh_options(force=False, silent=False):
    ...
    if os.path.isfile(target):
        if force:
            ...
            copy = _get_input(
                "Found a gmsh configuration file, do you wish to overwite it?", silent
            )
    ...
    if has_simnibs:
        if force:
            ...
            overwrite = _get_input(
                "Found another SimNIBS install, overwite it from the PATH?", silent
            )
...
def path_cleanup(scripts_dir, shell_type="bash"):
    ...
    if sys.platform in ["linux", "darwin"]:
        ...
        for key, value in simnibs_env_vars.items():
            ...
            with winreg.OpenKey(
                winreg.HKEY_CURRENT_USER, "Environment", access=winreg.KEY_WRITE
            ) as reg:
...
def links_setup(install_dir):
    ...
    linkfun(os.path.join(install_dir, "examples"), os.path.join(SIMNIBSDIR, "examples"))
    linkfun(os.path.join(install_dir, "simnibs"), SIMNIBSDIR)
    linkfun(
        os.path.join(install_dir, "resources"), os.path.join(SIMNIBSDIR, "resources")
    )
    linkfun(
...
def setup_shortcut_icons(scripts_dir, force=False, silent=False):
    ...
    if sys.platform == "darwin":
        ...
        gmsh_icon = os.path.join(
            SIMNIBSDIR, "_internal_resources", "icons", "gmsh", "logo.png"
        )
        simnibs_icon = os.path.join(
    ...
    if sys.platform == "win32":
        ...
        _create_shortcut(
            ...
            os.path.abspath(
                os.path.join(scripts_dir, "..", "documentation", "index.html")
            ),
        )
        _create_shortcut(
...
def _create_shortcut(
    shortcut_name, target_path, comment="", icon=None, mime_type=None, arguments=None
):
    if sys.platform == "win32":
...
def _create_apps(install_dir):
    ...
    _create_app(
        ...
        os.path.join(
            SIMNIBSDIR, "_internal_resources", "icons", "simnibs", "gui_icon.icns"
        ),
        plist,
    )
    ...
    for icns in glob.glob(
        os.path.join(SIMNIBSDIR, "_internal_resources", "icons", "gmsh", "*.icns")
    ):
...
def _create_app(app_name, executable, icon, plist):
    ...
    if not os.path.isdir(macos_dir):
        os.mkdir(macos_dir)
    _copy_and_log(icon, os.path.join(resouces_dir))
    # Write a simnibs_gui and a gmsh executable
    _write_unix_sh(executable, os.path.join(macos_dir, "simnibs_gui"))
        ...
        if _is_associated(ext):
            if force:
                ...
                associate[ext] = _get_input(
                    f'Found other association for "{ext}" files, overwrite it?', silent
                )
        else:
            associate[ext] = True
...
def _is_associated(ext):
    if sys.platform == "win32":
        ...
        ret = subprocess.run(
            "assoc " + ext, shell=True, capture_output=True, text=True, errors="replace"
        )
        if ret.returncode == 0:
...
def fix_qtconf(install_dir):
    ...
    if len(fns) < 1:
        print(
            f"Warning: no qt.conf file found within {install_dir}, cannot setup PyQt5"
        )
        return
    import configparser
    ...
    if len(prepath) > 0:
        ...
        if not os.path.isdir(plugin_path):
            plugin_path = os.path.join(
                prepath, "simnibs_env", "Library", "plugins", "platforms"
            )
        if not os.path.isdir(plugin_path):
            import glob

            print(os.path.join(prepath, "**", "plugins", "platforms"))
            plugin_path = glob.glob(
                os.path.join(prepath, "**", "plugins", "platforms"), recursive=True
            )[-1]
...
if GUI:
    ...
    class PostInstallGUI(QtWidgets.QDialog):
        def __init__(
            ...
            if self.copy_gmsh_options:
                gmsh_options_cb.toggle()
            ...
            if sys.platform == "win32":
                associate_cb = QtWidgets.QCheckBox(
                    "Associate .msh, .stl and .geo files"
                )
        ...
        def set_add_shortcut_icons(self, new_value):
            ...
            if not new_value:
                QtWidgets.QMessageBox.warning(
                    self, "SimNIBS", "SimNIBS icons will not be found in the Start Menu"
                )
...
def _fix_kernel_config(
    jupyter_core_paths: list[str], path_to_system_python: str
) -> None:
...
def uninstall(install_dir):
    ...
    if sys.platform == "win32":
        ...
        conda_uninstaller = os.path.join(
            install_dir, "miniconda3", "Uninstall-Miniconda3.exe"
        )
...
def main():
    ...
    if big_sur:
        ...
        try:
            ...
            ctypesloader_fn = os.path.join(
                os.path.split(OpenGL.__file__)[0], "platform", "ctypesloader.py"
            )
    ...
    parser = argparse.ArgumentParser(
        prog="postinstall_simnibs",
        description="Optional post-installation procedures for SimNIBS ",
    )
    parser.add_argument(
        "-d", "--target_dir", required=True, help="SimNIBS install directory"
    )
    parser.add_argument(
        "-f",
        "--force",
    ...
    parser.add_argument(
        "--no-copy-gmsh-options",
        dest="copy_gmsh_options",
        action="store_false",
        help="Do not copy gmsh options",
    )
    parser.add_argument(
        "--no-add-to-path",
        dest="add_to_path",
        action="store_false",
        help="Do not add SimNBIS to system PATH",
    )
    parser.add_argument(
        "--no-shortcut-icons",
        dest="add_shortcut_icons",
        action="store_false",
        help="Do not create start menu shortcuts",
    )
    parser.add_argument(
        "--no-associate-files",
        dest="associate_files",
        action="store_false",
        help="Do not create file associations",
    )
    parser.add_argument(
        "-u",
        "--uninstall",
