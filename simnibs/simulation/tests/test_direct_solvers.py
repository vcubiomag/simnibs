...
def create_matrix(dim, alpha=0.95, smallest_coef=0.1, largest_coef=0.9):
    """Based o scikit-learn make_sparse_spd_matrix"""
    rng = np.random.default_rng(seed=42)
    chol = -np.eye(dim)
    aux = rng.random((dim, dim))
    aux[aux < alpha] = 0
    aux[aux > alpha] = smallest_coef + (largest_coef - smallest_coef) * rng.random(
        np.sum(aux > alpha)
    )
    aux = np.tril(aux, k=-1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            "mumps",
            marks=pytest.mark.skipif(
                sys.platform != "darwin", reason="PETSc only built with MUMPS on macos."
            ),
        ),
        pytest.param(
            "preonly",
...
@pytest.mark.skipif(
    sys.platform == "darwin", reason="Intel MKL Pardiso not available on Mac OS X"
)
