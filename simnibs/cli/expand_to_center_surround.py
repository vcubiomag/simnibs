...
def parse_arguments(argv):
    parser = argparse.ArgumentParser(
        prog="expand_to_center_surround",
    ...
    parser.add_argument(
        "--radius_surround", dest="radius_surround", default=[50.0], nargs="+"
    )
    parser.add_argument(
        "--pos_dir_1stsurround", dest="pos_dir_1stsurround", default=None, nargs="+"
    )
    parser.add_argument("--N", dest="N", default=4, type=int)
    parser.add_argument("--multichannel", dest="multichannel", action="store_true")
    parser.add_argument(
        "--phis_surround", dest="phis_surround", default=None, nargs="+"
    )
...
def main():
    ...
    mat = scipy.io.loadmat(
        os.path.abspath(os.path.realpath(os.path.expanduser(args.S)))
    )
