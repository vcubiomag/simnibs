...
def writeResults(
    ...
    if threshold is not None:
        ...
        posteriors[:, thresholdStructureNumber] = (
            posteriors[:, thresholdStructureNumber] > threshold
        )
    ...
    writeImage(
        os.path.join(savePath, scanName + "_crispSegmentation.nii"),
    ...
    for contrastNumber, imageFileName in enumerate(imageFileNames):
        ...
        # Bias field
        writeImage(
            os.path.join(savePath, scanName + "_biasField.nii"),
        ...
        writeImage(
            os.path.join(savePath, scanName + "_biasCorrected.nii"),
    ...
    if savePosteriors:
        ...
        for i, name in enumerate(names):
            ...
            writeImage(
                os.path.join(posteriorPath, name + ".nii"), pvol, cropping, exampleImage
            )

    # Compute volumes in mm^3
    volumeOfOneVoxel = np.abs(
        np.linalg.det(exampleImage.transform_matrix.as_numpy_array[0:3, 0:3])
    )
