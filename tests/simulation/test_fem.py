...
from simnibs.simulation import fem, analytical_solutions
...
class TestFEMSystem:
    ...
    def test_set_up_tms(self, tms_sphere, rdm, mag):
    ...
    def test_dirichlet_problem_sphere(self, sphere3_msh, rdm):
        m = sphere3_msh.crop_mesh(elm_type=4)
        cond = np.ones(len(m.elm.tetrahedra))
    ...
    def test_solve_petsc(self, tms_sphere, rdm, mag):
        m, cond, dAdt, E_analytical = tms_sphere
        S = fem.TMSFEM(m, cond)
        b = S.assemble_rhs(dAdt)
        x = S.solve(b)
    ...
    def test_solve_dirichlet_petsc(self, cube_msh, rdm, mag):
        m = cube_msh
    ...
    def test_solve_assemble_neumann_tag(self, cube_msh, rdm, mag):
        m = cube_msh
        cond = np.ones(m.elm.nr)
    ...
    def test_solve_assemble_neumann_nodes(self, current_type, cube_msh, rdm, mag):
        m = cube_msh
        cond = np.ones(m.elm.nr)
        cond[m.elm.tag1 > 5] = 25
        cond = mesh_io.ElementData(cond)
    ...
    def test_solve_assemble_aniso(self, cube_msh, rdm, mag):
        m = cube_msh
...
class TestTDCS:
    def test_tdcs_petsc(self, cube_msh, rdm, mag):
        m = cube_msh
        cond = np.ones(m.elm.nr)
        cond[m.elm.tag1 > 5] = 1e3
        cond = mesh_io.ElementData(cond)
    ...
    def test_tdcs_petsc_3_el(self, cube_msh, rdm, mag):
        m = cube_msh
        areas = m.nodes_areas()
        cond = np.ones(m.elm.nr)
        cond[m.elm.tag1 > 5] = 1e3
    ...
    def test_tdcs_petsc_3_el_mp(self, cube_msh, rdm, mag):
        m = cube_msh
        cond = np.ones(m.elm.nr)
        cond[m.elm.tag1 > 5] = 1e3
        cond = mesh_io.ElementData(cond)
...
class TestTDCSNeumann:
    def test_tdcs_neumann_petsc(self, cube_msh, rdm, mag):
        m = cube_msh
        cond = np.ones(m.elm.nr)
        cond[m.elm.tag1 > 5] = 1e3
        cond = mesh_io.ElementData(cond)
    ...
    def test_tdcs_neumann_3_el(self, cube_msh, rdm, mag):
        m = cube_msh
        cond = np.ones(m.elm.nr)
        cond[m.elm.tag1 > 5] = 1e3
        cond = mesh_io.ElementData(cond)
...
class TestTMS:
    def test_tms_dadt(self, tms_sphere, rdm, mag):
        m, cond, dAdt, E_analytical = tms_sphere
    ...
    def test_tms_coil(self, mock_set_up, tms_sphere, rdm, mag):
        m, cond, dAdt, E_analytical = tms_sphere
    ...
    @patch.object(fem, "_get_da_dt_from_coil")
    def test_tms_coil_parallel(self, mock_set_up, tms_sphere, rdm, mag):
...
class TestDipole:
    ...
    def test_single_dipole(
        self, source_model, dipole_direction, pos_target, sphere3_msh, rdm, mag
    ):
    ...
    def test_many_dipoles(self, sphere3_msh, rdm, mag):
