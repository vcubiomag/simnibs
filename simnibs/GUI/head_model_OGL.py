...
YELLOW = QtGui.QColor.fromCmykF(0.0, 0.0, 1.0, 0.0)
BLUE = QtGui.QColor.fromCmykF(0.72, 0.52, 0.0, 0.0)
RED = QtGui.QColor.fromCmykF(0.0, 1.0, 1.0, 0.0)
GREEN = QtGui.QColor.fromCmykF(1.0, 0.0, 1.0, 0.0)
BLACK = QtGui.QColor.fromCmykF(0.0, 0.0, 0.0, 1.0)
GRAY = QtGui.QColor.fromCmykF(0.0, 0.0, 0.0, 0.34)
WHITE = QtGui.QColor.fromCmykF(0.0, 0.0, 0.0, 0.0)
PURPLE = QtGui.QColor.fromCmykF(0.0, 1.0, 0.0, 0.5)
DARK_RED = QtGui.QColor.fromCmykF(0.0, 1.0, 1.0, 0.45)
WHEAT = QtGui.QColor.fromCmykF(0.0, 0.09, 0.27, 0.04)
...
class GLHeadModel(QtWidgets.QOpenGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.skin_color = [135.0 / 255, 204.0 / 255, 238.0 / 255]
        self.gm_color = [160.0 / 255, 160.0 / 255, 160.0 / 255]
    ...
    def mouseDoubleClickEvent(self, event):
        if isinstance(self.skin_surf, surface.Surface):
            ...
            if self.intersect_point is not None:
                self.indicator = self.drawIndicator(
                    self.intersect_point, self.intersect_normal
                )
                self.update()
    ...
    def drawModel(self, surf_name, field=None):
        ...
        node_normals = numpy.array(
            [normal for normal in rendered_surf.nodes_normals], dtype="float32"
        )
        ...
        if self.hasField:
            ...
            b[octile] = 4 * norm_field[octile] + 0.5

            octile = (norm_field > 0.125) & (norm_field <= 3 * 0.125)
            g[octile] = 4 * norm_field[octile] - 0.5
            b[octile] = 1
            ...
            node_colors = (
                r * red[:, numpy.newaxis]
                + b * blue[:, numpy.newaxis]
                + g * green[:, numpy.newaxis]
            ).T
        else:
    ...
    def drawElectrode(self, elec, color=GREEN):
        ...
        if len(elec.plug) != 0:
            plug_center = (
                center
                + elec.plug[0].centre[1] * y_axis
                + elec.plug[0].centre[0] * x_axis
                + normal
            )
            if elec.plug[0].shape == "rect":
                self.rectangle(
                    plug_center, normal, x_axis, y_axis, elec.plug[0].dimensions
                )
        ...
        for point, name in zip(points, names):
            ...
            if normal is None or len(normal) == 0 or point is None or len(point) == 0:
                ...
                if bool(GLUT.glutBitmapString):
                    ...
                    GLUT.glutBitmapString(GLUT.GLUT_BITMAP_HELVETICA_12, name.encode())
                    GL.glEnable(GL.GL_LIGHTING)
    ...
    def drawPointAndDirs(self, matrix, color=GREEN, fn_coil=""):
        ...
        if fn_coil != "":
            ...
            if coil_mesh.elm.nr == 0:
                ...
                GL.glDrawElements(
                    GL.GL_TRIANGLES,
                    len(rendered_surf.tr_nodes) * 3,
                    GL.GL_UNSIGNED_INT,
                    numpy.ravel(rendered_surf.tr_nodes),
                )
        ...
        cyl_height = 20
        cyl_radius = 0.7
        z_dir = numpy.array([0.0, 0.0, 1.0], dtype="float64")
    ...
    def addElectrodeToList(self, electrode_struct, number):
        ...
        self.electrodes_objects[number] = self.drawElectrode(
            center, dimensions, shape, color
        )
    ...
    def get_dAdtField(self, matsimnibs, fn_coil):
        ...
        if fn_coil.endswith(".nii") or fn_coil.endswith(".nii.gz"):
            ...
            if self.skin_surf is not None:
                field_skin = (
                    numpy.linalg.norm(
                        TmsCoil.from_file(fn_coil).get_a_field(
                            numpy.array(self.skin_surf.nodes), M
                        ),
                        axis=1,
                    )
                    * 1e7
            ...
            if self.gm_surf is not None:
                field_gm = (
                    numpy.linalg.norm(
                        TmsCoil.from_file(fn_coil).get_a_field(
                            numpy.array(self.gm_surf.nodes), M
                        ),
                        axis=1,
                    )
                    * 1e7
    ...
    def drawHeatMapScale(self, field):
        ...
        if bool(GLUT.glutBitmapString):
            ...
            GLUT.glutBitmapString(
                GLUT.GLUT_BITMAP_HELVETICA_12, f"{max(field): .2f}".encode()
            )

            GL.glRasterPos2i(-32, -58)
            GLUT.glutBitmapString(
                GLUT.GLUT_BITMAP_HELVETICA_12, f"{min(field): .2f}".encode()
            )

            GL.glRasterPos2i(-5, -58)
            GLUT.glutBitmapString(GLUT.GLUT_BITMAP_HELVETICA_12, "dA/dt (V/m)".encode())
