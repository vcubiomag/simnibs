...
class Samseg:
    def __init__(
        ...
        atlasDir,
        savePath,
        userModelSpecifications=None,
        userOptimizationOptions=None,
    ...
    def register(self):
        ...
        if self.transformedTemplateFileName is None:
            ...
            self.imageToImageTransformMatrix, optimizationSummary = (
    ...
    def postProcess(self):
        ...
        if self.saveMesh:
            ...
            deformedAtlasFileName = os.path.join(
                self.savePath, scanName + "_meshCollection.txt.gz"
            )
    ...
    def saveWarpField(self, filename):
        ...
        matricesFileName = os.path.join(
            self.savePath, "template_coregistrationMatrices.mat"
        )
        if not os.path.isfile(matricesFileName):
            matricesFileName = os.path.join(
                self.savePath, "base", "template_coregistrationMatrices.mat"
            )
        ...
        # write the warp file
        fs.Warp(coordmap, source=imageGeom, target=templateGeom, affine=matrix).write(
            filename
        )
    ...
    def getDownSampledModel(self, atlasFileName, downSamplingFactors):
        ...
        for contrastNumber in range(numberOfContrasts):
            ...
            downSampledImageBuffers[:, :, :, contrastNumber] = self.imageBuffers[
                :: downSamplingFactors[0],
                :: downSamplingFactors[1],
                :: downSamplingFactors[2],
                contrastNumber,
        ...
        downSamplingTransformMatrix = np.pad(
            downSamplingTransformMatrix, (0, 1), mode="constant", constant_values=0
        )
        ...
        # Get the mesh
        downSampledMesh, downSampledInitialDeformationApplied = (
    ...
    def initializeGMM(self):
        ...
        self.classFractions, _ = kvlGetMergingFractionsTable(
            self.modelSpecifications.names, self.modelSpecifications.sharedGMMParameters
        )
        ...
        for multiResolutionLevel in range(numberOfMultiResolutionLevels):
            ...
            for iterationNumber in range(maximumNumberOfIterations):
                ...
                while True:
                    ...
                    downSampledGaussianPosteriors, minLogLikelihood = (
                ...
                logger.info("iterationNumber: %d" % iterationNumber)
                logger.info(
                    "maximalDeformationApplied: %.4f" % maximalDeformationApplied
                )
                logger.info("=======================================================")
                self.visualizer.show(
                    mesh=downSampledMesh,
                ...
                historyOfCost.append(
                    minLogLikelihoodTimesDeformationPrior + minLogGMMParametersPrior
                )
                self.visualizer.plot(
                    historyOfCost[1:],
            ...
            if self.saveHistory:
                ...
                levelHistory["downSampledTransformMatrix"] = (
                ...
                levelHistory["initialNodePositionsInTemplateSpace"] = (
                    initialNodePositionsInTemplateSpace
                )
                levelHistory["finalNodePositionsInTemplateSpace"] = (
                    finalNodePositionsInTemplateSpace
                )
