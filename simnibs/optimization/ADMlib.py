...
def recipcode(rv, jv, rs, ks, A):
    ...
    prec = (
        10 ** (-3)
    )  # this number determines the accuracy of H-primary evaluation higher accuracy=slower time increases ~log(accuracy)
    ...
    rp = np.transpose(
        np.c_[np.transpose(rs), np.ones([ncoil])]
    )  # pads coil positions to 4 by ncoil
...
def ADM(rv, jv, rs, ks, A, coildir):
    ...
    # generate copies of coil
    rp = np.transpose(
        np.c_[np.transpose(raux), np.ones([ncoil])]
    )  # pads coil positions to 4 by ncoil
    robs = np.array(np.zeros([3, ntarget]))
    for i in range(0, npos):
        st = i * ncoil
    ...
    for i in range(0, npos):
        ...
        for j in range(0, Nj):
            kp = np.matmul(
                A[0:3, 0:3, i], kaux[:, :, j]
            )  # for flat coils you only need the 3rd element
...
def resamplecoil(rs, ks, N, Nj, coildir):
    ...
    for i in range(0, Nj):
        ...
        ks2[2, :, i] = ks[2, :]
        ks2[0, :, i] = (
            ks[0, :] * coildir[1, i] + ks[1, :] * coildir[0, i]
        )  # unnecessary ks is z oriented
        ks2[1, :, i] = (
            -ks[0, :] * coildir[0, i] + ks[1, :] * coildir[1, i]
        )  # unnecessary ks is z oriented
    ...
    Zd = 0.6127016653792583 * 0.5 * (Zp - Zm)
    Zm = Zm - Zd
    Zp = Zp + Zd
    ...
    XX = (
        0.5
        * (interpnodes[int(N[0] * (N[0] - 1) / 2) : int(N[0] * (N[0] + 1) / 2)] + 1.0)
        * (Xp - Xm)
        + Xm
    )
    YY = (
        0.5
        * (interpnodes[int(N[1] * (N[1] - 1) / 2) : int(N[1] * (N[1] + 1) / 2)] + 1.0)
        * (Yp - Ym)
        + Ym
    )
    ZZ = (
        0.5
        * (interpnodes[int(N[2] * (N[2] - 1) / 2) : int(N[2] * (N[2] + 1) / 2)] + 1.0)
        * (Zp - Zm)
        + Zm
    )
        ...
        for k in range(0, N[2]):
            for j in range(0, N[1]):
                for i in range(0, N[0]):
                    L = Lx[i, :] * Ly[j, :] * Lz[k, :]
                    kaux[0, int(i + (j + k * N[1]) * N[0]), kk] = np.inner(
                        ks2[0, :, kk], L
                    )
                    kaux[1, int(i + (j + k * N[1]) * N[0]), kk] = np.inner(
                        ks2[1, :, kk], L
                    )
                    kaux[2, int(i + (j + k * N[1]) * N[0]), kk] = np.inner(
                        ks2[2, :, kk], L
                    )
    return raux, kaux
