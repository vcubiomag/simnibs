...
def potential_3layers_surface_electrodes(
    radii, conductivities, anode_pos, cathode_pos, p, nbr_polynomials=50
):
    ...
    cathode_pos = (
        np.sqrt(cathode_pos[0] ** 2 + cathode_pos[1] ** 2 + cathode_pos[2] ** 2) * 1e-3,
        np.arccos(
            cathode_pos[2]
    ...
    anode_pos = (
        np.sqrt(anode_pos[0] ** 2 + anode_pos[1] ** 2 + anode_pos[2] ** 2) * 1e-3,
        np.arccos(
            anode_pos[2]
    ...
    A = lambda n: ((2 * n + 1) ** 3 / (2 * n)) / (
        ((b_over_s + 1) * n + 1) * ((s_over_t + 1) * n + 1)
        + (b_over_s - 1)
        * (s_over_t - 1)
        * n
        * (n + 1)
    ...
    skull_region = np.where((p_r[:, 0] > radius_brain) * (p_r[:, 0] <= radius_skull))[0]
    skin_region = np.where((p_r[:, 0] > radius_skull) * (p_r[:, 0] <= radius_skin))[0]
    inside_sphere = np.where((p_r[:, 0] <= radius_skin))[0]
    outside_sphere = np.where((p_r[:, 0] > radius_skin))[0]

    cos_theta_a = np.cos(cathode_pos[1]) * np.cos(p_r[:, 1]) + np.sin(
        cathode_pos[1]
    ) * np.sin(p_r[:, 1]) * np.cos(p_r[:, 2] - cathode_pos[2])
    cos_theta_b = np.cos(anode_pos[1]) * np.cos(p_r[:, 1]) + np.sin(
        anode_pos[1]
    ) * np.sin(p_r[:, 1]) * np.cos(p_r[:, 2] - anode_pos[2])
    ...
    with warnings.catch_warnings():
        ...
        for ii in range(1, nbr_polynomials):
            ...
            coefficients[ii, skull_region] = np.nan_to_num(
                S(n) * (p_r[skull_region, 0] / radius_skin) ** n
                + U(n) * (p_r[skull_region, 0] * radius_skin) ** (-n - 1)
            )

            coefficients[ii, skin_region] = np.nan_to_num(
                T(n) * (p_r[skin_region, 0] / radius_skin) ** n
                + W(n) * (p_r[skin_region, 0] * radius_skin) ** (-n - 1)
            )

        potentials[inside_sphere] = np.nan_to_num(
            np.polynomial.legendre.legval(
                cos_theta_a[inside_sphere], coefficients[:, inside_sphere], tensor=False
            )
            - np.polynomial.legendre.legval(
                cos_theta_b[inside_sphere], coefficients[:, inside_sphere], tensor=False
            )
        )
...
def potential_homogeneous_dipole(
    sphere_radius, conductivity, dipole_pos, dipole_moment, detector_positions
):
    ...
    second_term = (
        ...
        * (
            detector_positions
            + (detector_positions * r0 * cos_phi[:, None] - R * dipole_pos)
            / (R + rp - r0 * cos_phi)[:, None]
        )
    )
    V = dipole_moment.dot(
        (2 * (detector_positions - dipole_pos) / (rp**3)[:, None] + second_term).T
    ).T
...
def B_outside_sphere(sphere_radius, dipole_pos, dipole_moment, detector_positions):
    ...
    assert np.all(np.linalg.norm(detector_positions, axis=1) > sphere_radius), (
        "All points must be outside the sphere"
    )

    assert np.all(np.linalg.norm(dipole_pos) < sphere_radius), (
    ...
    for ii, r in enumerate(detector):
        ...
        grad_F = (
            ...
            + 2 * norm_a
            + 2 * norm_r
        ) * r - (norm_a + 2 * norm_r + norm_a ** (-1) * a.dot(r)) * r_0

        B[ii, :] = (
            (4 * np.pi * 1e-7)
            / (4 * np.pi * F**2)
            * (F * np.cross(moment, r_0) - np.dot(np.cross(moment, r_0), r) * grad_F)
        )
...
def tms_E_field(dipole_pos, dipole_moment, didt, positions):
    ...
    if dipole_pos.shape != dipole_moment.shape:
        raise ValueError(
            "List of dipole position and moments should have the samelengths"
        )
    mu0_4pi = 1e-7
    ...
    for m, r2 in zip(dm, dp):
        ...
        grad_F = (
            norm_a**2 / norm_r2 + 2 * norm_a + 2 * norm_r2 + r2_dot_a / norm_a
        ...
        E += (
            ...
            * (
                F * np.cross(r1, m)
                - np.cross(np.sum(m * grad_F, axis=1)[:, None] * r1, r2)
            )
        )
...
def lpmn(m: int, n: int, x: float | npt.ArrayLike):
    ...
    for i in range(m + 1):
        for j in range(i + 1, n):
            pmn[i, j + 1] = ((2 * j + 1) * x * pmn[i, j] - (j + i) * pmn[i, j - 1]) / (
                j - i + 1
            )
...
def potential_dipole_3layers(
    ...
    assert np.all(dipole_pos_norm < radii[0]), (
        "All dipoles must be inside the inner sphere"
    )
    ...
    def d(nbrs, f1, f2):
        d_n = (
            ((nbrs + 1) * xi + nbrs) * ((nbrs * xi) / (nbrs + 1) + 1)
            + (1 - xi)
            * ((nbrs + 1) * xi + nbrs)
            * (f1 ** (2 * nbrs + 1) - f2 ** (2 * nbrs + 1))
    ...
    potentials = np.nan_to_num(
        np.sum(
            ((2 * nbrs[:, None] + 1) / nbrs[:, None] * b[None] ** (nbrs[:, None] - 1))[
                :, None
            ]
            * ((xi * (2 * nbrs + 1) ** 2) / (d(nbrs, f1, f2) * (nbrs + 1)))[
                :, None, None
            ]
            * (
                nbrs[:, None, None] * m_r[None, None] * P[0]
                - m_t[None, None] * P[1] * cos_beta[None]
            ),
            axis=0,
        )
    )
