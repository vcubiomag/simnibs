...
def vchoosek(v, k):
    ...
    while 1:
        ...
        y[(a - 1) : (b), k - 1] = v[
            0, (Index[k - 1] - 1) : (nv)
        ]  # Write the K.th column
        a = b + 1  # Move the write pointer

        newLoop = np.sum(Index < Limit)
        if newLoop == 0:  # All columns are filled:
            break  # Ready!
        Index[(newLoop - 1) : (k)] = Index[newLoop - 1] + np.linspace(
            1, k - newLoop + 1, k - newLoop + 1
        )
...
def allVL1leq(n, L1):
    ...
    for i_L1 in range(L1 + 1):
        ...
        s = vchoosek(
            np.linspace(1, n + i_L1 - 1, n + i_L1 - 1) * np.ones([1, n + i_L1 - 1]),
            n - 1,
        )  # 1:n+L1-1
...
def NRMSD(x, ref):
    ...
    for i_data in range(N_data):
        NRMSD[i_data] = (
            100
            * np.sqrt(
                1.0 / N_points * np.sum((data[:, i_data] - data_ref) ** 2, axis=0)
            )
            / delta
        )
...
def fit_betapdf(
    data, BETATOL=0, PUNI=0, PLOT=0, VISI=1, filename=[], xlabel="$x$", ylabel="$p(x)$"
):
    ...
    if BETATOL > 0:
        ...
        p_beta, q_beta, a_beta, ab_beta = scipy.stats.beta.fit(
            data, floc=a_beta, fscale=b_beta - a_beta
        )
    else:
    ...
    beta_mean, beta_var = scipy.stats.beta.stats(
        p_beta, q_beta, loc=a_beta, scale=(b_beta - a_beta), moments="mv"
    )
    ...
    if PUNI > 0:
        a_uni = scipy.stats.beta.ppf(
            (1 - PUNI) / 2, p_beta, q_beta, loc=a_beta, scale=b_beta - a_beta
        )
        b_uni = scipy.stats.beta.ppf(
            (1 + PUNI) / 2, p_beta, q_beta, loc=a_beta, scale=b_beta - a_beta
        )
    ...
    if PLOT:
        ...
        n, bins, patches = plt.hist(
            data, bins=10, normed=1, color=[1, 1, 0.6], alpha=0.5
        )
        ...
        beta_y = scipy.stats.beta.pdf(
            beta_x, p_beta, q_beta, loc=a_beta, scale=b_beta - a_beta
        )
        ...
        if PUNI > 0:
            uni_x = np.hstack(
                [
                    a_beta,
                    a_uni - 1e-6 * (b_uni - a_uni),
                    np.linspace(a_uni, b_uni, 100),
                    b_uni + 1e-6 * (b_uni - a_uni),
                    b_beta,
                ]
            )
        ...
        ax.set_xlim(
            a_beta - 0.05 * (b_beta - a_beta), b_beta + 0.05 * (b_beta - a_beta)
        )
