...
class SimuList(object):
    ...
    def cond2elmdata(self, mesh=None, excentricity_scale=None, logger_level=20):
        ...
        if self.anisotropy_type == "scalar":
            ...
            logger.log(
                level,
                "Using anisotropic direct conductivities based on the file: {0}".format(
    ...
    def _write_conductivity_to_hdf5(self, fn_hdf5, path="cond/"):
        ...
        with h5py.File(fn_hdf5, "a") as f:
            ...
            g.create_dataset(
                "names", data=np.array([c.name for c in self.cond], dtype=np.string_)
            )
            g.create_dataset(
                "distribution_types",
                data=np.array(
                    [c.distribution_type for c in self.cond], dtype=np.string_
                ),
            )
...
class TDCSLIST(SimuList):
    ...
    def _prepare(self):
        ...
        if len(self.unique_channels) != len(self.currents):
            raise ValueError(
                "Number of unique channels is not equal to the number ofcurrent values"
            )
...
class ELECTRODE(object):
    ...
    def shape(self, s):
        if s in ["rectangle", "rect", "ellipse", "custom", ""]:
            ...
            raise ValueError("Electrode shape must be'rect', 'ellipse', 'custom' or ''")
...
class TDCSLEADFIELD(LEADFIELD):
    ...
    def run(self, cpus=1, allow_multiple_runs=False, save_mat=True):
        ...
        if self.interpolation:
            assert self.interpolation_tissue, (
                "Interpolation tissues must be specified when interpolating solution"
            )
            assert not contains_surface(self.interpolation_tissue), (
                "Interpolation tissues must be volumes"
            )
            if self.tissues:
                assert not contains_volume(self.tissues), (
            ...
            assert contains_volume(self.tissues) ^ contains_surface(self.tissues), (
