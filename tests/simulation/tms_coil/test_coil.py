...
def testcoil_ccd(test_data_dir):
    return test_data_dir / "testcoil.ccd"


@pytest.fixture(scope="module")
def testcoil_nii_gz(test_data_dir):
    return test_data_dir / "testcoil.nii.gz"


@pytest.fixture(scope="module")
def testcoil_nii_gz_B(test_data_dir):
    return test_data_dir / "testcoilB.nii.gz"
...
class TestWriteNifti:
    def test_from_ccd_to_nifti(self, tmp_path: Path, testcoil_ccd, testcoil_nii_gz):
        coil = TmsCoil.from_file(str(testcoil_ccd))
    ...
    def test_from_ccd_to_nifti_undividable_ranges(
        self, tmp_path: Path, testcoil_ccd, testcoil_nii_gz
    ):
        coil = TmsCoil.from_file(str(testcoil_ccd))
        coil.write_nifti(
            str(tmp_path / "test.nii.gz"),
        ...
        np.testing.assert_array_equal(
            nifti_coil.limits, np.array([[-100, 101], [-100, 101], [-100, 101]])
        )
        ...
        np.testing.assert_allclose(
            np.linalg.norm(coil.get_a_field(test_positions, np.eye(4)), axis=1),
            np.linalg.norm(nifti_coil.get_a_field(test_positions, np.eye(4)), axis=1),
            rtol=0.04,
        )

    def test_from_ccd_to_nifti_dividable_ranges(
        self, tmp_path: Path, testcoil_ccd, testcoil_nii_gz
    ):
        coil = TmsCoil.from_file(str(testcoil_ccd))
        coil.write_nifti(
        ...
        nifti_coil = coil.from_nifti(str(tmp_path / "test.nii.gz"))
        np.testing.assert_array_equal(
            nifti_coil.limits, np.array([[-100, 100], [-100, 100], [-100, 100]])
        )
