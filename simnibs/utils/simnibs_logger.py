...
def register_excepthook(logger):
    def log_excep(exc_type, exc_value, exc_traceback):
        ...
        logger.critical("Uncaught exception", exc_info=(exc_type, exc_value, None))
...
def format_time(running_time):
    ...
    """
    hrs = np.uint16(np.floor(running_time / (60.0**2)))
    mts = np.uint16(np.floor(running_time / 60.0 - hrs * 60))
    sec = np.uint16(np.round(running_time - hrs * 60.0**2 - mts * 60.0))
