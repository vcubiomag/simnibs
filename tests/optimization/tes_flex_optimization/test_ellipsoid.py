...
class TestEllipsoid:
    def test_fit(self):
        ...
        rotmat = np.array(
            [
                [1, 0, 0],
                [0, np.cos(30.0 / 180.0 * np.pi), np.sin(30.0 / 180.0 * np.pi)],
                [0, -np.sin(30.0 / 180.0 * np.pi), np.cos(30.0 / 180.0 * np.pi)],
            ]
        )
        ...
        eli_coords = ellipsoid.ellipsoid2cartesian(
            coords=coords_sphere, return_normal=False
        )
        ...
        np.testing.assert_allclose(
            np.abs(ellipsoid.rotmat),
            np.abs(ellipsoid_fit.rotmat),
            atol=1e-10,
    ...
    def test_transformations(self):
        ...
        for coords_test in coords_eli:
            ...
            assert np.isclose(
                coords_test, coords_cart2eli
            ).all(), "Cartesian coordinates of ellipsoidal transformation differ compared to reference"

            # transform back and forth (jacobian <-> cartesian)
            coords_cart2jac = ellipsoid.cartesian2jacobi(
                coords=coords_eli2cart
            )  # coords_eli2cart
            coords_jac2cart = ellipsoid.jacobi2cartesian(coords=coords_cart2jac)

            assert np.isclose(
                coords_eli2cart, coords_jac2cart, rtol=1e-4
            ).all(), "Cartesian coordinates of jacobian transformation differ compared to reference"
    ...
    def test_geodesic_distance(self):
        ...
        rotmat = np.array(
            [
                [-0.09566444, -0.04310434, 0.99447993],
        ...
        for _start_jacobi in start_jacobi:
            ...
            # traveling distance
            distance = 20.0 * np.ones(len(alpha))

            # start point in cartesian coordinates
            start_cart = np.tile(
                ellipsoid.jacobi2cartesian(coords=_start_jacobi), (len(alpha), 1)
            )

            # determine geodesic distances
            end = ellipsoid.get_geodesic_destination(
                start=start_cart,
                alpha=alpha,
                distance=distance,
                n_steps=400,
            ...
            diff_maxrel = (
                np.max(np.abs(np.linalg.norm(start_cart - end, axis=1) - distance))
                / distance[0]
            )
