...
def parseArguments(argv):
    ...
    parser.add_argument(
        "-m",
        metavar="head_model.msh",
        required=True,
        help="Head mesh where the electrode positions are to be calculated",
    )
    parser.add_argument(
        "-o", help="output base file name", metavar="10_10_positions", required=True
    )
    parser.add_argument(
        "-i",
        help="input csv file, must define all 4 fiducials",
    ...
    parser.add_argument(
        "-LPA", help="Left preauricular point coordinates x y z", nargs=3, type=float
    )
    parser.add_argument(
        "-RPA", help="Right preauricular point coordinates x y z", nargs=3, type=float
    )
    parser.add_argument(
        "--NE_cap",
        help="Wether to calculate positions for the Neuroelectrics cap",
        action="store_true",
    )
    parser.add_argument("--version", action="version", version=__version__)

    args = parser.parse_args()
    if args.i is None and any(
        [args.Iz is None, args.Iz is None, args.LPA is None, args.RPA is None]
    ):
        raise ValueError(
            "either a .csv-file has to be passed or all four fiducials have to be defined"
        )
...
def main():
    ...
    eeg = eeg_positions.FindPositions(
        args.Nz, args.Iz, args.LPA, args.RPA, surf, args.NE_cap
    )
