...
def tms_sphere(sphere3_msh):
    ...
    cond.mesh = m
    stimulator = tms_coil.tms_stimulator.TmsStimulator(
        "Example Stimulator", "Example Stimulator Brand", 100e6
    )
    dipole_elm = tms_coil.tms_coil_element.DipoleElements(
        stimulator=stimulator, points=dipole_pos[None], values=dipole_moment[None]
    )
...
class TestOnlineFEM:
    ...
    @pytest.mark.parametrize(
        "nearest_neighbor", [pytest.param(False), pytest.param(True)]
    )
    # loop over nearest neighbor or SPR interpolation
    @pytest.mark.parametrize(
        "dirichlet_correction", [pytest.param(False), pytest.param(True)]
    )
    @pytest.mark.slow
    def test_tes_cube(
        self, cube_msh, rdm, mag, solver_type, nearest_neighbor, dirichlet_correction
    ):
        m = cube_msh
        ...
        electrode_i._electrode_arrays[0].electrodes[0].node_coords = m.nodes.node_coord[
            idx_node_1
        ]
        electrode_i._electrode_arrays[1].electrodes[0].node_coords = m.nodes.node_coord[
            idx_node_2
        ]
        ...
        # e-field
        e = ofem.update_field(
            electrode=[electrode_i], dirichlet_correction=dirichlet_correction
        )[0][0]

        # check results
        avg_v1 = np.sum(ofem.v[idx_node_1] * n_areas[idx_node_1]) / np.sum(
            n_areas[idx_node_1]
        )
        avg_v2 = np.sum(ofem.v[idx_node_2] * n_areas[idx_node_2]) / np.sum(
            n_areas[idx_node_2]
        )
    ...
    # loop over nearest neighbor or SPR interpolation
    @pytest.mark.parametrize(
        "dirichlet_correction_detailed", [pytest.param(False), pytest.param(True)]
    )
    @pytest.mark.slow
    def test_tes_cube_dirichlet_channel(
        self, cube_msh, solver_type, dirichlet_correction_detailed
    ):
        ...
        cond.value[elm_bari[:, 0] < 0] = (
            2.0  # divide into 2 halves with different conductivities
        )

        idx_dirinode = np.argmin(np.sum(m.nodes.node_coord**2, axis=1))
        idx_node_1_l = np.where(
            (m.nodes.node_coord[:, 1] >= 50.0) * (m.nodes.node_coord[:, 0] < 0.0)
        )[0]  # top plate left
        idx_node_1_r = np.where(
            (m.nodes.node_coord[:, 1] >= 50.0) * (m.nodes.node_coord[:, 0] >= 0.0)
        )[0]  # top plate right

        idx_node_2_l = np.where(
            (m.nodes.node_coord[:, 1] <= -50.0) * (m.nodes.node_coord[:, 0] < 0.0)
        )[0]  # bottom plate left
        idx_node_2_r = np.where(
            (m.nodes.node_coord[:, 1] <= -50.0) * (m.nodes.node_coord[:, 0] >= 0.0)
        )[0]  # bottom plate right
        ...
        electrode_i._electrode_arrays[0].electrodes[1].node_coords = m.nodes.node_coord[
            idx_node_1_r
        ]
        ...
        electrode_i._electrode_arrays[1].electrodes[1].node_coords = m.nodes.node_coord[
            idx_node_2_r
        ]
        ...
        # check results
        avg_v1_l = np.sum(ofem.v[idx_node_1_l] * n_areas[idx_node_1_l]) / np.sum(
            n_areas[idx_node_1_l]
        )
        avg_v1_r = np.sum(ofem.v[idx_node_1_r] * n_areas[idx_node_1_r]) / np.sum(
            n_areas[idx_node_1_r]
        )
        avg_v2_l = np.sum(ofem.v[idx_node_2_l] * n_areas[idx_node_2_l]) / np.sum(
            n_areas[idx_node_2_l]
        )
        avg_v2_r = np.sum(ofem.v[idx_node_2_r] * n_areas[idx_node_2_r]) / np.sum(
            n_areas[idx_node_2_r]
        )
    ...
    @pytest.mark.parametrize("fill", [pytest.param(False), pytest.param(True)])
    # loop over nearest neighbor or SPR interpolation
    ...
    def test_tms_sphere(
        ...
        point_cloud = onlinefem.FemTargetPointCloud(
            m, center_points, nearest_neighbor=nearest, fill_nearest=fill
        )

        # prepare and setup OnlineFEM
        ofem = onlinefem.OnlineFEM(
            m,
            "TMS",
            roi=[point_cloud],
            coil=coil,
