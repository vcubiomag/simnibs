...
class TestCV:
    def test_k_fold_cv_regression(self):
        ...
        A = np.eye(50)
        x = np.ones(50)
        r = functools.partial(adaptive._tikhonov, alpha=0.0)
        b = A.dot(x)
        noise = 0.5 * np.ones(50)
        b_noise = b + noise
        ...
        eps = adaptive._k_fold_cv_regression(
            A, b_noise, regression, error_eq=error_eq, k=10
        )
        assert np.isclose(eps, 0.5)
...
class TestAdaptive:
    def test_adaptive_expand(self, uniform_dist):
        ...
        reg, _ = adaptive.run_reg_adaptive_grid(
            pdftype, pdfshape, limits, function, data_poly_ratio=2
        )
