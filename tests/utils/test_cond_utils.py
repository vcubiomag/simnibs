...
class TestCond2Elmdata:
    ...
    def test_anisotropic(self, sphere3_msh, tensor):
        ...
        affine = np.array(
            [[-1, 0, 0, 128], [0, 1, 0, -128], [0, 0, 1, -127], [0, 0, 0, 1]]
        )
...
class TestExcentricity:
    def test_excentricity_no_change(self):
        t = np.tile(np.array([10.0, 5, 1]), [10, 1])
        t[1] = [1, 1, 1]
        t2 = simnibs.utils.cond_utils._adjust_excentricity(t, 0.5)
        assert np.allclose(t2, t)

    def test_excentricity_to_iso(self):
        t = np.tile(np.array([10.0, 5, 1]), [10, 1])
        t[1, :] = 50 ** (1.0 / 3.0)
        t2 = simnibs.utils.cond_utils._adjust_excentricity(t, 0)
        assert np.allclose(t2.prod(axis=1), t.prod(axis=1))
        assert np.allclose(t2, 50 ** (1.0 / 3.0))

    def test_iso_to_excentric(self):
        t = np.tile(np.array([1.0, 1, 1]), [10, 1])
        t *= np.arange(1, 11)[:, None]
        t2 = simnibs.utils.cond_utils._adjust_excentricity(t, 0.99)
        assert np.allclose(t2, t)
...
class TestTensorVisualization:
    def test_visualization(self, tensor):
        ...
        assert np.allclose(
            fields[0].value / factor[:, None], eig_val[max_eig] * eig_vec[:, max_eig]
        )
        assert np.allclose(
