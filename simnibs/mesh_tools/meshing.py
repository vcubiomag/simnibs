...
def image2mesh(
    ...
    if not np.allclose(shearing, np.eye(3), atol=1.0e-5):
    ...
    if (
        type(cell_size) is np.ndarray
        or type(facet_size) is np.ndarray
        or type(facet_distance) is np.ndarray
    ):
        if type(cell_size) is np.ndarray:
        ...
        if type(facet_distance) is np.ndarray:
            ...
            facet_distance = facet_distance * np.ones_like(
                image, dtype=np.float32, order="F"
            )
...
def _mesh_surfaces(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for surf, sd in zip(surfaces, subdomains):
            if len(sd) != 2:
                raise MeshingError(
                    "Subdomain speficifation must have exactly 2 entries"
                )
...
def remesh(
    ...
    for i in range(2):
        adj_labels_fixed[adj_labels[:, i] != tr_labels, 1] = adj_labels[
            adj_labels[:, i] != tr_labels, i
        ]
    ...
    for i, t1 in enumerate(tags):
        for j, t2 in enumerate(tags[i + 1 :]):
            tr_interface = (adj_labels[:, 0] == t1) * (adj_labels[:, 1] == t2)
...
def _get_test_nodes(
    ...
    * required as _get_spikes_from_conn_matrix is rather slow
    ...
    if not fast_track:
        ...
        idx_elm, _, adj_diff = _get_elm_and_new_tag(
            tag, adj_tets, 3, return_diffmat=True
        )
        ...
        idx_node = scipy.stats.mode(
            faces[faces_elm].reshape((-1, 9)), axis=1, keepdims=True
        )[0]
        if len(idx_node) > 0:
        ...
        m_surf.elm.add_triangles(
            faces[idx_surface_tri, :] + 1, np.ones(len(idx_surface_tri), dtype=int)
        )
...
def _get_spikes_from_conn_matrix(conn_nodes, idx_test_nodes, nneighb):
    ...
    * nodes belonging to more complex surface configurations
    ...
    for i, j in zip(start_idx, stop_idx):
        ...
        idx_spike_nodes = np.append(
            idx_spike_nodes, _get_spikes(conn_nodes, idx_hlp, nneighb)
        )
    ...
    for idx_node in idx_spike_nodes:
        ...
        while len(idx_tets) > 0:
            ...
            while len(idx_group) > len_idx_group:
                ...
                idx_group = idx_group[
                    (idx_group != -1) * (tag_spk[idx_group] == tag_group)
                ]
        ...
        spike_data.append(
            (
                idx_node + 1,
                map_new_old[idx_spike_tets],
                tag_spk[idx_spike_tets[0]],
                tag_neigh[0],
            )
        )
...
def _get_candidates_for_splitting(sp_dat, node_number_list, idx_surf_nodes):
    ...
    for sp in sp_dat:
        ...
        if len(idx_sp_tets) < 2:
            logger.info(
                " _get_candidates_for_splitting: spike with one tetrahedron should not occur at this stage"
            )
            ...
            idx = np.where(cn == 1)[0]
            if len(idx) == 2:
                sp2_unique_nodes = np.append(
                    sp2_unique_nodes, un[idx].reshape((1, 2)), axis=0
                )
                sp2_spike_tets = np.append(
                    sp2_spike_tets, idx_sp_tets.reshape((1, 2)), axis=0
                )
            else:
            ...
            if len(idx) == 0:
                ...
                logger.info(
                    " _get_candidates_for_splitting: weird spike with two fully connected 2nd nodes"
                )
...
def _select_splits_from_candidates(splittest, node_number_list, node_coord, tag_org):
    ...
    for sp in splittest:
        ...
        idx_orgtets = np.where(
            np.any(node_number_list == idx_n1, axis=1)
            * np.any(node_number_list == idx_n2, axis=1)
        )[0]
        if len(idx_orgtets) == 0:
            raise ValueError(
                "The two nodes are not connected! This happens sporadically. Running the meshing again (charm subID --mesh) should solve the problem."
            )
        ...
        if np.all(
            np.abs(cosalpha - 0.5) < 0.3
        ):  # 0.3 is a magic number determined during initial testing
            splitlist.append(sp)

    return splitlist


def _combine_small_spikes(
    sp2_unique_nodes, sp2_spike_tets, adj_tets, node_number_list, tag_org, nr_nodes
):
    """
    ...
    for i in un:
        ...
        un2, cn2 = np.unique(
            node_number_list[sp2_spike_tets[idx_spikes]], return_counts=True
        )
        idx_sp_nodes = un2[cn2 == 4]

        if len(idx_sp_nodes) == 2:
            # get the correct tags and append to splitlist
            _, sp_dat_hlp = _get_new_tag_for_spikes(
                idx_sp_nodes - 1, adj_tets, node_number_list, tag_org, nr_nodes
            )
            splitlist.append(
                (idx_sp_nodes[0], idx_sp_nodes[1], sp_dat_hlp[1][3], sp_dat_hlp[0][3])
            )
...
def _split_spikes(m, splitlist):
    ...
    for sp in splitlist:
        ...
        idx_tets1, idx_tets2 = m.split_tets_along_line(
            idx_n1, idx_n2, return_tetindices=True
        )
    ...
    * updates tag1 and tag2 of m.elm
    ...
    for i in np.unique(m.elm.tag1):
        ed = mesh_io.ElementData.from_data_grid(
            m, (vol[:] == i).astype(np.float32), affine, "", order=1
        )
...
def update_tag_from_tet_neighbors(m, faces, tet_faces, adj_tets, nr_iter=12):
    ...
    * updates tag1 and tag2 of m.elm
    ...
    for i in range(nr_iter):
        ...
        idx_elm, new_tag, adj_diff = _get_elm_and_new_tag(
            tag, adj_tets, 2, return_diffmat=True
        )
        ...
        facenodes_elm = facenodes_elm[:, :5][np.diff(facenodes_elm) == 0].reshape(
            (-1, 2)
        )
        ...
        idx_surface_tri, face_node_diff = _get_surfaces(
            faces, tet_faces, adj_tets, tag, m.nodes.nr
        )[:2]
        idx = (
            np.max(face_node_diff[facenodes_elm], axis=1) > 0
        )  # face_node_diff > 0 indicates a surface defect
        idx_elm = idx_elm[idx]
        new_tag = new_tag[idx]
        tag[idx_elm] = new_tag
...
def update_tag_from_surface(
    m, faces, tet_faces, adj_tets, do_splits=False, fast_track=False
):
    ...
    * for do_splits=True: variables faces, tet_faces, adj_tets will
    ...
    idx_surface_tri, face_node_diff, nneighb, conn_nodes = _get_surfaces(
        faces, tet_faces, adj_tets, m.elm.tag1, m.nodes.nr
    )

    # get to-be-tested surface nodes (uses heuristics to lower the number of candidate nodes)
    idx_test_nodes = _get_test_nodes(
        faces,
        tet_faces,
        adj_tets,
        idx_surface_tri,
    ...
    logger.info(
        "     Testing "
        + str(np.sum(idx_test_nodes))
        + " nodes (matrix: "
        + str(conn_nodes.shape)
        + ", "
        + str(conn_nodes.nnz)
        + " entries)"
    )
    ...
    tag_buff = m.elm.tag1
    m.elm.tag1, sp_dat = _get_new_tag_for_spikes(
        idx_spike_nodes, adj_tets, m.elm[:], m.elm.tag1, m.nodes.nr
    )
    logger.info("   Relabled " + str(np.sum(m.elm.tag1 != m.elm.tag2)) + " tets")
    m.elm.tag2[:] = m.elm.tag1
    ...
    if do_splits:
        ...
        splittest, sp2_tets, sp2_uniquenodes = _get_candidates_for_splitting(
            sp_dat, m.elm.node_number_list, idx_surf_nodes
        )
        #   step 2: determine the spikes that will be split from the candidates
        splitlist = _select_splits_from_candidates(
            splittest, m.elm.node_number_list, m.nodes.node_coord, tag_buff
        )
        ...
        splitlist += _combine_small_spikes(
            sp2_uniquenodes,
            sp2_tets,
            adj_tets,
            m.elm.node_number_list,
...
def _remove_spikes(m, label_img, affine, label_GM=2, label_CSF=3):
    ...
    m = update_tag_from_label_img(
        m, adj_tets, label_img, affine, label_GM=label_GM, label_CSF=label_CSF
    )
    ...
    logger.info(
        "Done Removing Spikes: Total number of relabled tets: "
        + str(np.sum(m.elm.tag1[: len(tag_buff)] != tag_buff))
        + "; Number of split tets: "
        + str(len(m.elm.tag1) - len(tag_buff))
    )
...
def _fix_labels(m, label_img):
    ...
    if n_dropped:
        ...
        logger.warning(
            "{} small region(s) dropped during meshing. Check label numbers in mesh!".format(
                n_dropped
            )
        )
...
def create_mesh(
    ...
    thickness[thickness < 0.5] = 100  # set background thickness to some large value
    ...
    size_field = _sizing_field_from_thickness(label_img, thickness, elem_sizes)
    distance_field = _sizing_field_from_thickness(label_img, thickness, facet_distances)
    del thickness
    ...
    if smooth_size_field:
        size_field = size_field ** (
            1 / 3
        )  # compress high values to preserve edges a bit better
        kernel = (
            smooth_size_field
            + 1
            - np.abs(np.arange(-smooth_size_field, smooth_size_field + 1, 1))
        )
        kernel = kernel / np.sum(kernel)
        for i in range(3):
            size_field = scipy.ndimage.convolve1d(
                size_field, kernel, axis=i, mode="constant", cval=0.0, origin=0
            )
    ...
    logger.info("Time to prepare meshing: " + format_time(time.time() - start))
    ...
    logger.info("Time to mesh: " + format_time(time.time() - start))
    ...
    logger.info("Time to post-process mesh: " + format_time(time.time() - start))
    return m
...
def apply_cream_layer(label_img, size_field, distance_field, cream_thickness):
    ...
    for i in range(cream_thickness):
        ...
        size_field[mask_dil] = scipy.ndimage.grey_dilation(
            skin_and_cream_size_field, footprint=footprint
        )[mask_dil]
        distance_field[mask_dil] = scipy.ndimage.grey_dilation(
            skin_and_cream_distance_field, footprint=footprint
        )[mask_dil]
