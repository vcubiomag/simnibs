...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "--out_fill", help=out_descrip, type=check_out_fill, default="nan"
    )
    parser.add_argument(
        "--method", help=type_descrip, default="linear", choices=["linear", "assign"]
    )
...
def main():
    ...
    for ed in msh.elmdata:
        ...
        logger.info(
            "Interpolating field: {0} and writing to file: {1}".format(
    ...
    for nd in msh.nodedata:
        ...
        f = nd.interpolate_scattered(points, out_fill=args.out_fill, squeeze=False)
        if f.ndim == 1:
            f = f[:, None]
        np.savetxt(fn_out, f, delimiter=",")
