...
def sensor_setup(nr_theta=4, nr_phi=4, r_min=0.1, r_max=0.9, nr_r=5):
    ...
    x = (r * np.sin(theta) * np.cos(phi)).reshape(
        -1,
    )
    y = (r * np.sin(theta) * np.sin(phi)).reshape(
        -1,
    )
    z = (r * np.cos(theta)).reshape(
        -1,
    )
...
class TestDipole:
    ...
    def test_symmetry_z_axis(self, quadrant_points):
        v = analytical_solutions.potential_homogeneous_dipole(
            1.0, 1.0, [0, 0, 0], [0, 0, 1], quadrant_points
        )
        np.testing.assert_almost_equal(
            v[quadrant_points[:, 2] > 0], -v[quadrant_points[:, 2] < 0]
        )

    def test_tilt_dipole_x(self, quadrant_points):
        v = analytical_solutions.potential_homogeneous_dipole(
            1.0, 1.0, [0, 0, 0], [1, 0, 0], quadrant_points
        )
        np.testing.assert_almost_equal(
            v[quadrant_points[:, 0] > 0], -v[quadrant_points[:, 0] < 0]
        )

    def test_move_dipole_xyz(self, quadrant_points):
        v = analytical_solutions.potential_homogeneous_dipole(
            1.0, 1.0, [0.5, 0.5, 0.5], [1, 0, 0], quadrant_points
        )
...
class TestDipole3Layers:
    def test_dipole_source_at_center(self):
        ...
        v = analytical_solutions.potential_dipole_3layers(
            [0.8, 0.9, 1], 1.0, 1.0, [0, 0, 0], [0, 0, 1], points, 1
        )
        v_simple = 3 / (4 * np.pi * 1e-6) * (1 / np.sqrt(2))
    ...
    def test_symmetry_z_axis(self, quadrant_points):
        v = analytical_solutions.potential_dipole_3layers(
            [0.8, 0.9, 1], 1.0, 1.0, [0, 0, 0], [0, 0, 1], quadrant_points
        )
        np.testing.assert_almost_equal(
            v[quadrant_points[:, 2] > 0], -v[quadrant_points[:, 2] < 0]
        )
    ...
    def test_tilt_dipole_x(self, quadrant_points):
        v = analytical_solutions.potential_dipole_3layers(
            [0.8, 0.9, 1], 1.0, 1.0, [0, 0, 0], [1, 0, 0], quadrant_points
        )
        np.testing.assert_almost_equal(
            v[quadrant_points[:, 0] > 0], -v[quadrant_points[:, 0] < 0]
        )
    ...
    def test_move_dipole_xyz(self, quadrant_points):
        v = analytical_solutions.potential_dipole_3layers(
            [0.8, 0.9, 1], 1.0, 1.0, [0.3, 0.3, 0.3], [1, 0, 0], quadrant_points
        )
    ...
    def test_rotate_and_move_dipole(self, quadrant_points):
        vx = analytical_solutions.potential_dipole_3layers(
            [0.8, 0.9, 1.0], 1.0, 1.0, [0.5, 0, 0], [1, 0, 0], quadrant_points
        )
        M = np.array([[0, 1, 0], [1, 0, 0], [0, 0, 1]])
    ...
    def test_compare_homogeneous(self, quadrant_points):
        ...
        v1 = analytical_solutions.potential_dipole_3layers(
            [0.8, 0.9, 2], 1.0, 1.0, [0, 0, 0.5], [1, 1, 1], p, 500
        )
        v2 = analytical_solutions.potential_homogeneous_dipole(
            2.0, 1.0, [0, 0, 0.5], [1, 1, 1], p
        )
    ...
    def test_radial_dipole(self, quadrant_points):
        ...
        m_bar = 1.0
        m_tilde = 1.0 / 1.536
        V_3layers = analytical_solutions.potential_dipole_3layers(
            [0.87, 0.92, 1.0],
            1.0,
            0.0125,
            [b_bar, 0, 0],
        ...
        V_homo = analytical_solutions.potential_homogeneous_dipole(
            1.0, 1.0, [b_tilde, 0, 0], [m_tilde, 0, 0], quadrant_points
        )
    ...
    def test_tangential_dipole(self, quadrant_points):
        ...
        V_homo = analytical_solutions.potential_homogeneous_dipole(
            1.0, 1.0, [b_tilde, 0, 0], [0, m_tilde, 0], quadrant_points
        )
        assert np.allclose(V_3layers, V_homo, rtol=5e-2)
...
def test_reciprocity():
    ...
    pot = analytical_solutions.potential_3layers_surface_electrodes(
        radii, cond, electrodes[0, :], electrodes[1, :], dipole_pos
    )
    E_field = (pot[1:4] - pot[4:7]) / 2e-13
    dipole_pot = analytical_solutions.potential_homogeneous_dipole(
        1.0, 1.0, dipole_pos[0], dipole_moment, electrodes
    )
...
class Test_B_field:
    def test_center_source(self, quadrant_points):
        ...
        B = analytical_solutions.B_outside_sphere(
            0.8, [0, 0, 0], [1, 0, 0], quadrant_points
        )
    ...
    def test_radial_source(self, quadrant_points):
        ...
        B = analytical_solutions.B_outside_sphere(
            0.8, [0.2, 0.1, 0.1], [0.2, 0.1, 0.1], quadrant_points
        )
        np.testing.assert_allclose(B, np.zeros(B.shape))
...
class Test_TMS_field:
    def test_radial_source(self, quadrant_points):
        ...
        E = analytical_solutions.tms_E_field(
            dipole_pos, dipole_moment, 1, quadrant_points
        )
        E_simple = (
            -1e-7
            * np.cross(dipole_pos - quadrant_points, dipole_moment)
