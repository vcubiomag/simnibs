...
def two_spheres_image():
    ...
    R_sp1 = (
        radius_sp1 - R_sp1
    ) / cortexth_sp1 + 0.5  # add 0.5 to center central surface at given radius
    R_sp1[R_sp1 > 1] = 1
    R_sp1[R_sp1 < 0] = 0
...
class TestMarchingCube:
    def test_with_spheres(self):
        ...
        _, EC = marching_cube(
            img, vox2mm, level=0.5, step_size=2, only_largest_component=False
        )
        assert EC == 4

        CS, EC = marching_cube(
            img, vox2mm, level=0.5, step_size=2, only_largest_component=True
        )
        assert EC == 2
        assert np.allclose(
            np.linalg.norm(CS.nodes.node_coord - center_sp1, axis=1), radius_sp1, atol=2
        )
