...
class TestGetOptGrid:
    def test_get_pos(self, sphere3_msh):
        pos, normals = tms_optimization._create_grid(sphere3_msh, [90, 0, 0], 0, 10, 2)
        R = np.linalg.norm(pos - [94.60, -2.81, 5.79], axis=1)
        assert np.all(R <= 10)
        assert np.isclose(np.max(R), 10, atol=1.5)
        assert np.allclose(
            pos / np.linalg.norm(pos, axis=1)[:, None], normals, rtol=1e-1, atol=1e-1
        )
        dists = scipy.spatial.distance_matrix(pos, pos)
        dists += 1e6 * np.eye(len(pos))
        assert np.allclose(np.min(dists, axis=1), 2, atol=1e-1)

    def test_rotate_system(self):
        R = np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]])
        matrices = tms_optimization._rotate_system(R, [-90, 90], 90)
        assert len(matrices) == 3
        # fix these
        assert np.allclose(matrices[0], np.array([[0, 1, 0], [0, 0, -1], [-1, 0, 0]]))
        assert np.allclose(matrices[1], R)
        assert np.allclose(matrices[2], np.array([[0, -1, 0], [0, 0, -1], [1, 0, 0]]))
    ...
    def test_get_opt_grid(self, sphere3_msh):
        ...
        for i in range(n_pos):
            ...
            assert np.allclose(
                np.sum(A[0, :, :2] * B[-1, :, :2], axis=0), np.cos(np.deg2rad(20))
            )
    ...
    def test_get_opt_handle_dir(self, sphere3_msh):
        ...
        for i in range(n_pos):
            ...
            assert np.allclose(
                np.sum(A[0, :, :2] * B[-1, :, :2], axis=0), np.cos(np.deg2rad(120))
            )
            assert np.allclose(
...
def test_get_opt_grid_ADM(sphere3_msh):
    ...
    normal_component = np.sum(
        matrices[:3, 2, :] * centers / np.linalg.norm(centers, axis=0), axis=0
    )
    assert np.all(normal_component < -0.9)
    handle_dir = np.sum(matrices[:3, 1, :].T * [0, 0, 1], axis=1)
