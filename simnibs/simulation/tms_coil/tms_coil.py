...
class TmsCoil(TcdElement):
    ...
    def from_tcd_dict(cls, coil: dict, validate=True):
        ...
        if validate:
            with open(
                file_finder.templates.tcd_json_schema, "r", encoding="utf-8"
            ) as fid:
    ...
    def write_nifti(
        ...
        sample_positions, dims, new_limits = self.get_sample_positions(
            limits, resolution
        )
    ...
    def get_sample_positions(
        ...
        new_limits = np.array([[x[0], x[-1]], [y[0], y[-1]], [z[0], z[-1]]])

        return (
            np.array(np.meshgrid(x, y, z, indexing="ij"))
            .reshape((3, -1))
            .T.astype(np.float64),
            dims,
            new_limits,
