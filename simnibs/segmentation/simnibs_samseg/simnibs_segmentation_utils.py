...
def _calculateSegmentationLoop(
    ...
    if cat_opts is not None:
        ...
        wm_probs = np.zeros(biasCorrectedImageBuffers.shape[0:3], dtype=np.float32)
        gm_probs = np.zeros(biasCorrectedImageBuffers.shape[0:3], dtype=np.float32)
        csf_probs = np.zeros(biasCorrectedImageBuffers.shape[0:3], dtype=np.float32)
        normalizer = np.zeros(biasCorrectedImageBuffers.shape[0:3], dtype=np.float32)
    ...
    if cat_opts is not None:
        cat_scan_and_masks = _prep_scans_and_masks(
            [wm_probs, gm_probs, csf_probs],
            normalizer,
            maxIndices,
            FreeSurferLabels,
            cat_mask_dict,
...
def _prep_scans_and_masks(
    tissue_prob_list, normalizer, maxInds, FreeSurferLabels, mask_dict
):
