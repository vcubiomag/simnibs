...
def calc_B(J, n_voxels, affine, calc_res=2.0, mask=False):
...
def _bs_ft(J, res, pad=None):
    ...
    kx, ky, kz = np.meshgrid(
        *[
            2 * np.pi * np.fft.fftfreq(d + p, res * 1e-3)
            for d, p in zip(J.shape[:3], pad)
        ],
        indexing="ij",
        sparse=True,
    )
    ...
    Jx_fft = np.fft.fftn(
        np.pad(J[..., 0], [(p // 2,) for p in pad], "constant", constant_values=0)
    )
    By = 1j * K * kz * Jx_fft
    Bz = -1j * K * ky * Jx_fft
    del Jx_fft
    ...
    Jy_fft = np.fft.fftn(
        np.pad(J[..., 1], [(p // 2,) for p in pad], "constant", constant_values=0)
    )
    Bx = -1j * K * kz * Jy_fft
    Bz += 1j * K * kx * Jy_fft
    del Jy_fft
    ...
    Jz_fft = np.fft.fftn(
        np.pad(J[..., 2], [(p // 2,) for p in pad], "constant", constant_values=0)
    )
    Bx += 1j * K * ky * Jz_fft
    By += -1j * K * kx * Jz_fft
    del Jz_fft

    Bx = np.real(np.fft.ifftn(Bx))[
        pad[0] // 2 : -pad[0] // 2,
        pad[1] // 2 : -pad[1] // 2,
        pad[2] // 2 : -pad[2] // 2,
    ]
    By = np.real(np.fft.ifftn(By))[
        pad[0] // 2 : -pad[0] // 2,
        pad[1] // 2 : -pad[1] // 2,
        pad[2] // 2 : -pad[2] // 2,
    ]
    Bz = np.real(np.fft.ifftn(Bz))[
        pad[0] // 2 : -pad[0] // 2,
        pad[1] // 2 : -pad[1] // 2,
        pad[2] // 2 : -pad[2] // 2,
    ]
