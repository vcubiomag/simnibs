...
from simnibs.utils.simnibs_logger import (
    logger,
    register_excepthook,
...
use custom .ini-file to control tetrahedra sizes:
    ...
    parser = argparse.ArgumentParser(prog="meshmesh", usage=usage_text)
    parser.add_argument(
        "label_image", nargs="?", help="filename of the nifti label image"
    )
    ...
    parser.add_argument(
        "--voxsize_meshing",
        dest="voxsize_meshing",
        metavar="voxel_size_in_mm",
        default=None,
        nargs=1,
    ...
    parser.add_argument(
        "--sizing_field",
        dest="fn_sizing_field",
        default=None,
        nargs=1,
        metavar="size_field.nii.gz",
    ...
    parser.add_argument(
        "--debug_path", default=None, help="""Debug path to write debug output"""
    )
...
def main():
    ...
    if args.usesettings is None:
        args.usesettings = os.path.join(
            SIMNIBSDIR, "charm.ini"
    ...
    label_image = np.squeeze(
        label_nifti.get_fdata().astype(np.uint16)
    ...
    if args.voxsize_meshing is not None:
        ...
        if sf_image is not None:
            sf_image, _, _ = resample_vol(
                sf_image, label_affine, args.voxsize_meshing, order=order_upsampling
            )
        if order_upsampling == 0:
            label_image, label_affine, _ = resample_vol(
                label_image, label_affine, args.voxsize_meshing, order=0
            )
        else:
            labels = np.unique(label_image)
            best_tag_p, new_affine, _ = resample_vol(
                (label_image == labels[0]).astype(np.float32),
                label_affine,
                args.voxsize_meshing,
                order=1,
            )
            ...
            for i in labels[1:]:
                tag_p = resample_vol(
                    (label_image == i).astype(np.float32),
                    label_affine,
                    args.voxsize_meshing,
                    order=1,
                )[0]
    ...
    if sf_image is not None:
        sf_image, _, _ = crop_vol(
            sf_image, label_affine, label_image > 0, thickness_boundary=5
        )

    label_image, label_affine, _ = crop_vol(
        label_image, label_affine, label_image > 0, thickness_boundary=5
    )
