...
def testernie_nii(test_data_dir):
    return test_data_dir / "ernie_T1_ds5.nii.gz"


@pytest.fixture(scope="module")
def testmni_nii(test_data_dir):
    return test_data_dir / "MNI_test_ds5.nii.gz"
...
def testcubenoise_nii(test_data_dir):
    return test_data_dir / "cube_noise.nii.gz"


@pytest.fixture(scope="module")
def testcube_nii(test_data_dir):
    return test_data_dir / "cube.nii.gz"
...
def test_sanlm(tmpdir, testernie_nii):
    ...
    charm_utils._denoise_input_and_save(str(testernie_nii), denoised_scan)
...
def test_mni_affine(tmpdir, testmni_nii):
    ...
    estimated_trans_mat = charm_utils._init_atlas_affine(
        str(trans_scan_name), str(testmni_nii), trans_settings
    )
    np.testing.assert_allclose(
...
def test_atlas_affine(tmpdir, testmni_nii, testtemplate_nii, testaffinemesh_msh):
    ...
    charm_utils._register_atlas_to_input_affine(
        str(trans_scan_name),
        str(testtemplate_nii),
        str(testaffinemesh_msh),
        str(testaffinemesh_msh),
        str(testaffinemesh_msh),
        str(tmpdir),
...
def test_T1T2(tmpdir, testmni_nii):
    ...
    charm_utils._registerT1T2(
        str(trans_scan_name), str(testmni_nii), str(registered_scan_name)
    )
...
def test_segmentation(tmpdir, testcube_nii, testcubenoise_nii, testcubeatlas_path):
    ...
    param_estimates = charm_utils._estimate_parameters(
        str(seg_dir),
        str(testcubeatlas_path / "template.nii.gz"),
        str(testcubeatlas_path),
        [str(testcubenoise_nii)],
        seg_settings,
        str(testcubeatlas_path / "sharedGMMParameters.txt"),
        visualizer,
