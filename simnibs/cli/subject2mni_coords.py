...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "-s", "--csv", dest="csv", required=False, help=csv_descrip, default=None
    )
    parser.add_argument(
        "-o",
        "--out",
        ...
        "If not set, the values will be printed to the screen",
        default=None,
    )
    parser.add_argument(
        "--transformation_type",
        "-t",
        dest="t",
        help="(optional) Type of transformation to use.{0} Default: nonl".format(
            transf_types_str
        ),
        default="nonl",
...
def main():
    ...
    if fn_out is None:
        np.set_printoptions(
            precision=2, formatter={"float": lambda x: "{0:.2f}".format(x)}
        )
