...
def quadrature_jacobi_1D(N, b, a):
    ...
    t2 = np.sqrt(
        (4.0 * N_arr * (N_arr + a) * (N_arr + b) * (N_arr + a + b))
...
def quadrature_fejer2_1D(N):
    ...
    if N == 1:
        ...
        for i in range(0, N):
            ...
            for j in range(0, jhi):
                angle = 2.0 * float(j + 1) * theta[i]
                weights[i] = weights[i] - 2.0 * np.cos(angle) / float(
                    4 * (j + 1) ** 2 - 1
                )
                p = 2 * ((N + 1) // 2) - 1
...
def denorm(coords_norm, pdftype, gridshape, limits):
    ...
    for i_DIM in range(coords_norm.shape[1]):
        ...
        if pdftype[i_DIM] == "beta":
            coords[:, i_DIM] = (coords_norm[:, i_DIM] + 1) / 2 * (
                limits[1][i_DIM] - limits[0][i_DIM]
            ) + limits[0][i_DIM]
        # if gridtype[i_DIM] == 'hermite':
        if pdftype[i_DIM] == "norm" or pdftype[i_DIM] == "normal":
            coords[:, i_DIM] = (
                coords_norm[:, i_DIM] * gridshape[1][i_DIM] + gridshape[0][i_DIM]
            )
...
def norm(coords, pdftype, gridshape, limits):
    ...
    for i_DIM in range(coords.shape[1]):
        ...
        if pdftype[i_DIM] == "beta":
            ...
            coords_norm[:, i_DIM] = (
                coords_norm[:, i_DIM] / (limits[1][i_DIM] - limits[0][i_DIM]) * 2.0 - 1
            )

            # if gridtype[i_DIM] == 'hermite':
        if pdftype[i_DIM] == "norm" or pdftype[i_DIM] == "normal":
            coords_norm[:, i_DIM] = (
                coords[:, i_DIM] - gridshape[0][i_DIM]
            ) / gridshape[1][i_DIM]
    return coords_norm
    ...
    def __init__(self, pdftype, gridtype, gridshape, limits, N):
        ...
        for i_DIM in range(self.DIM):
            if self.gridtype[i_DIM] == "jacobi":  # jacobi polynomials
                knots_temp, weights_temp = quadrature_jacobi_1D(
                    self.N[i_DIM],
        ...
        # +- 3.000 * sigma -> 99.73%
        for i_DIM in range(self.DIM):
            if (self.pdftype[i_DIM] == "norm" or self.pdftype[i_DIM] == "normal") and (
                not (self.gridtype[i_DIM] == "hermite")
            ):
        ...
        self.coords = denorm(
            self.coords_norm, self.pdftype, self.gridshape, self.limits
        )
...
class sparsegrid:
    def __init__(
        ...
        self.order_sequence_type = (
            order_sequence_type  # 'lin', 'exp' type of order sequence (common: 'exp')
        )
            ...
            if self.order_sequence_type == "exp":  # order = 2**level + 1
                if self.gridtype[i_dim] == "fejer2":  # start with order = 1 @ level = 1
                    order_sequence.append(
                        2 ** (np.linspace(1, self.level[i_dim], self.level[i_dim])) - 1
                    )
                    order_sequence[i_dim][0] = 1
                elif (
                    self.gridtype[i_dim] == "patterson"
                ):  # start with order = 1 @ level = 0 [1,3,7,15,31,...]
                    order_sequence.append(
                        2
                        ** (
                            np.linspace(0, self.level[i_dim], self.level[i_dim] + 1) + 1
                        )
                        - 1
                    )
                else:  # start with order = 1 @ level = 0
                    order_sequence.append(
                        2 ** np.linspace(0, self.level[i_dim], self.level[i_dim] + 1)
                        + 1
                    )
                    order_sequence[i_dim][0] = 1
                ...
                if self.gridtype[i_dim] == "fejer2":  # start with level = 1 @ order = 1
                    order_sequence.append(
                        np.linspace(1, self.level[i_dim] + 1, self.level[i_dim] + 1)
                    )
                elif (
                    self.gridtype[i_dim] == "patterson"
                ):  # start with order = 1 @ level = 0 [1,3,7,15,31,...]
        ...
        if any("fejer2" in s for s in self.gridtype):
            if self.DIM == 1:
                l_level = np.array(
                    [np.linspace(1, self.level_max, self.level_max)]
                ).transpose()
            else:
            ...
            if self.DIM == 1:
                l_level = np.array(
                    [np.linspace(0, self.level_max, self.level_max + 1)]
                ).transpose()
            else:
        ...
        if self.interaction_order < self.DIM:
            if any("fejer2" in s for s in self.gridtype):
                self.l_level = self.l_level[
                    np.sum(self.l_level > 1, axis=1) <= self.interaction_order, :
                ]
            else:
                self.l_level = self.l_level[
                    np.sum(self.l_level > 0, axis=1) <= self.interaction_order, :
                ]
        ...
        for i_dim in range(self.DIM):
            for i_level in level_sequence[i_dim]:  # range(level[i_dim]+1):
                if self.gridtype[i_dim] == "jacobi":  # Jacobi polynomials
                    knots_l, weights_l = quadrature_jacobi_1D(
                        order_sequence[i_dim][i_level],
                        self.gridshape[0][i_dim] - 1,
                        self.gridshape[1][i_dim] - 1,
                    )
                    knots_l_1, weights_l_1 = quadrature_jacobi_1D(
                        order_sequence[i_dim][i_level - 1],
                        self.gridshape[0][i_dim] - 1,
                        self.gridshape[1][i_dim] - 1,
                    )
                if self.gridtype[i_dim] == "hermite":  # Hermite polynomials
                    knots_l, weights_l = quadrature_hermite_1D(
                        order_sequence[i_dim][i_level]
                    )
                    knots_l_1, weights_l_1 = quadrature_hermite_1D(
                        order_sequence[i_dim][i_level - 1]
                    )
                if self.gridtype[i_dim] == "patterson":  # Gauss-Patterson
                    knots_l, weights_l = quadrature_patterson_1D(
                        order_sequence[i_dim][i_level]
                    )
                    knots_l_1, weights_l_1 = quadrature_patterson_1D(
                        order_sequence[i_dim][i_level - 1]
                    )
                if self.gridtype[i_dim] == "cc":  # Clenshaw Curtis
                    knots_l, weights_l = quadrature_cc_1D(
                        order_sequence[i_dim][i_level]
                    )
                    knots_l_1, weights_l_1 = quadrature_cc_1D(
                        order_sequence[i_dim][i_level - 1]
                    )
                if self.gridtype[i_dim] == "fejer2":  # Fejer type 2
                    knots_l, weights_l = quadrature_fejer2_1D(
                        order_sequence[i_dim][i_level - 1]
                    )
                    knots_l_1, weights_l_1 = quadrature_fejer2_1D(
                        order_sequence[i_dim][i_level - 2]
                    )
        ...
        for i_l_level in range(self.l_level.shape[0]):
            ...
            for i_dim in range(self.DIM):
                knots_temp.append(
                    np.asarray(
                        dl_k[int(self.l_level[i_l_level, i_dim])][i_dim], dtype=float
                    )
                )
                weights_temp.append(
                    np.asarray(
                        dl_w[int(self.l_level[i_l_level, i_dim])][i_dim], dtype=float
                    )
                )
        ...
        while any(Point_number_list < 0):
            ...
            point_temp[np.sum(np.abs(dL_k_nf - dL_k_nf[0, :]), axis=1) < epsilon_k] = (
                point_no
            )
    ...
    def __init__(self, pdftype, gridshape, limits, N):
        ...
        for i_DIM in range(self.DIM):
            if self.pdftype[i_DIM] == "beta":
                self.coords_norm[:, i_DIM] = (
                    np.random.beta(
                        self.gridshape[0][i_DIM], self.gridshape[1][i_DIM], [self.N, 1]
                    )
                    * 2.0
                    - 1
                )[:, 0]
