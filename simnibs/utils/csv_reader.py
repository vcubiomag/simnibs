...
def read_csv_positions(fn):
    ...
    rows = rows_filtered
    try:
        coordinates = np.array([[float(d) for d in r[1:4]] for r in rows], dtype=float)
    except:
...
def write_csv_positions(
    filename, types, coordinates, name, extra=None, extra_cols=None, header=None
):
