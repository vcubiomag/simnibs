...
def parseArguments(argv):
    ...
    parser.add_argument(
        "-f", "--fixed", dest="fixed_scan", required=True, help="Fixed scan."
    )
    parser.add_argument(
        "-m", "--moving", dest="moving_scan", required=True, help="Moving scan."
    )
    parser.add_argument(
        "-dof",
        "--degrees_of_freedom",
        dest="dof",
        help="Degrees of freedom: 6 (rigid (default)) or 12 (affine).",
    )
    parser.add_argument(
        "-o",
        "--output",
        dest="output",
        required=True,
        help="Output name, including path.",
    )
...
def main():
    ...
    if int(args.dof) == 6:
        ...
        reg = gems.KvlAffineRegistration(
            -100, 300, 0, [2.0, 1.0, 0], 0, [4.0, 2.0, 0], True, 1.0, "b"
        )
        reg.read_images(args.fixed_scan, args.moving_scan)
        reg.initialize_transform()
        reg.register()
