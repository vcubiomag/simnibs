...
class Affine:
    ...
    def getTransformMatrix(
        ...
        scalingCenter = np.array(
            [self.scalingCenter[0], self.scalingCenter[1], self.scalingCenter[2], 1]
        ).reshape(-1, 1)
    ...
    def gridSearch(
        ...
        self.visualizer.show(
            images=self.image.getImageBuffer(), mesh=mesh, window_id=visualizerTitle
        )
        self.visualizer.show_movie(window_id=visualizerTitle)
    ...
    def setUp(self, initTransform, targetDownsampledVoxelSpacing, visualizer):
        ...
        voxelSpacing = np.sum(imageToWorldTransformMatrix[0:3, 0:3] ** 2, axis=0) ** (
            1 / 2
        )
        ...
        upSamplingTranformMatrix = np.diag(
            np.pad(downSamplingFactors, (0, 1), "constant", constant_values=(0, 1))
        )
