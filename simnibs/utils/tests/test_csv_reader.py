...
class TestCSV:
    def test_read_csv_generic(self):
        ...
        f.close()
        type_, coordinates, extra, name, extra_cols, header = (
            csv_reader.read_csv_positions(f.name)
        )
    ...
    def test_read_csv_fiducial_electrode(self):
        ...
        assert np.allclose(coordinates, [[1, 2, 3], [1.2, 2.4, 3.6], [1.2, 2.4, 7.1]])
        assert header == []
    ...
    def test_read_csv_coil(self):
        ...
        assert np.allclose(coordinates, [[1, 2, 3], [1.2, 2.4, 7.1], [1, 2, 3]])
        assert header == []
    ...
    def test_write_csv_generic(self):
        ...
        csv_reader.write_csv_positions(
            f.name, type_, coordinates, name, extra, extra_cols, header
        )
    ...
    def test_write_csv_electrode(self):
        ...
        coordinates = np.array([[1, 2, 3], [1.2, 2.4, 3.6], [1.2, 2.4, 7.1]])
        header = []
        ...
        with open(f.name, "r") as f:
            ...
            assert (
                f.readline().strip()
                == "Electrode,1.2,2.4,3.6,9.0,8.0,7.0,electrode,b2,b3"
            )
    ...
    def test_write_csv_coil(self):
        ...
        coordinates = np.array([[1, 2, 3], [1.2, 2.4, 7.1], [1, 2, 3]])
        header = []
        name = ["fiducial", None, "coil"]
        extra_cols = [["a2"], None, ["comment"]]
        extra = [None, None, np.array([4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 10], dtype=float)]
