...
def build(simnibs_dist_dir, developer_id=None):
    ...
    wheels = glob.glob(os.path.join(simnibs_dist_dir, f"simnibs-{version}*.whl"))
    if len(wheels) == 0:
    ...
    subprocess.run(
        f"conda env create -p {env_prefix} -f {env} --yes", check=True, shell=True
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="simnibs-pack", description="Prepare a SimNIBS intaller"
    )
    parser.add_argument(
        "dist_dir", help="Directory with the SimNIBS wheels to be packed"
    )
    parser.add_argument(
        "--developer-id",
        default=None,
