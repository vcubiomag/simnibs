...
class localite:
    ...
    def read(self, fn, markertype=None):
        ...
        )  # 'coil position' for untracked coils.
        ...
        # build TMSLIST
        tms_list = TMSLIST()
        for i, (im, descr, onset, stim_int, didt) in enumerate(
            zip(*self._parse_localite_xml(fn, markertype=markertype))
        ):
    ...
    def write(
        self, matsimnibs, xml_fn, names=None, overwrite=False, out_coord_space="RAS"
    ):
        """
        ...
        # check inputs
        assert matsimnibs.shape[:2] == (4, 4), (
            "Expecting array with shape (4, 4, N instrument marker)."
        )
        out_coord_space = out_coord_space.upper()
        assert out_coord_space in ["RAS", "LPS"], (
            f'out_coord_space={out_coord_space} is not one of ["RAS","LPS"].'
        )

        if not xml_fn.lower().endswith(".xml"):
            xml_fn += ".xml"

        assert not os.path.exists(xml_fn) or overwrite, (
            "File {fn} already exists. Remove or set overwrite=True."
        )
        ...
        with io.open(
            ...
            f.write(
                f"    <!--This InstrumentMarkerList was written by SimNIBS v{simnibs_v}-->\n"
            )
            ...
            for idx in range(n_ims):
                ...
                f.write(
                    "    "
                    + f'<InstrumentMarker alwaysVisible="false" index="{idx}" selected="false">\n'
                )
                f.write(
                    "    " * 2 + f'<Marker additionalInformation="" '
                    f'color="#ff0000" description="{name}" set="true">\n'
                )
                f.write("    " * 3 + "<Matrix4D \n")
                f.write(
                    "    " * 4
                    + 'data00="{:+1.17f}" data01="{:+1.17f}" '
    ...
    def _parse_localite_xml(self, im_path, markertype):
        ...
        if coord_system == "RAS":
            ...
            raise ValueError(
                f"Coordinate system {coord_system} not supported. Use 'RAS' or 'LPS'."
            )
        ...
        for marker_i in im_root.iter(markertype):
            ...
            if markertype == "InstrumentMarker":
                ...
                for _im_rv in im_rv:
                    ...
                    if _im_rv.get("key") == "valueA":
                        ...
                        if val in ["NaN", "0.0"]:
                            warnings.warn(
                                "Couldn't determine stimulation intensity. Defaulting to 1 A/µs"
                            )
                            val = 1e6
                        else:
                            val = float(val) * 1e6
            ...
            for im_index1 in range(4):
                for im_index2 in range(4):
                    marker_arr[0, im_index1, im_index2] = float(
                        matrix4d.get(f"data{str(im_index1)}{str(im_index2)}")
                    )
...
class brainsight:
        ...
        with open(fn, "r") as f:
            ...
            if version >= 14:
                ...
                if not coord_sys.lower().startswith("nifti:q:"):
                    raise ValueError(
                        f"Coordinate system '{coord_sys}' is not supported. "
                    )
            else:
            ...
            if len(data_targets):
                names_targets, matsimnibs_targets = self._transform_brainsight(
                    data_targets, col_names_targets
                )
            else:
                names_targets, matsimnibs_targets = [], None

            if len(data_samples):
                names_samples, matsimnibs_samples = self._transform_brainsight(
                    data_samples, col_names_samples
                )
            else:
    ...
    def write(self, matsimnibs, fn, names=None, overwrite=False):
        ...
        assert not os.path.exists(fn) or overwrite, (
            f"File {fn} already exists. Remove or set overwrite=True."
        )
    ...
    def _transform_brainsight(self, data, col_names):
        ...
        for pos, i in zip(data, range(len(data))):
            ...
            m = [
                [p_dict["m0n0"], p_dict["m1n0"], p_dict["m2n0"], p_dict["Loc. X"]],
...
class ant:
        ...
        for k in keys1:
            if len(ImageInfo[k]) != len(ImageInfo2[k]):
                ...
                if not np.all(np.isclose(ImageInfo[k], ImageInfo2[k], atol=1e-05)):
                    warnings.warn(
                        "these info field has varying entries: "
                        + str(k)
                        + " "
                        + str(ImageInfo[k])
                        + " "
    ...
    def _mrk_template(self):
        mrk_template = {
            "Configuration": {
                "SourceInfo": {"AppName": "SimNIBS", "AppVersion": str(simnibs_v)},
                "AssociatedImageInfo": {
                    "NIfTI_quatern": [0.0, 0.0, 0.0],
        ...
        if targets is None:
            ...
            for t in targets:
                ...
                if coiltarget is None:
                    ...
                    p.matsimnibs = np.asarray(
                        coiltarget["CoilPose"]["RigidTransformMatrix4x4"]
                    )
    ...
    def write(self, tms_lists, fn, subpath=None, imageinfo=None):
        ...
        for tms_list in tms_lists:
            for p in tms_list.pos:
                ...
                pos_dict["CoilTarget"]["CoilPose"]["RigidTransformMatrix4x4"] = (
                    p.matsimnibs.tolist()
                )
