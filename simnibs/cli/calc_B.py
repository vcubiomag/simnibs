...
def parse_arguments(argv):
    parser = argparse.ArgumentParser(
        prog="calc_B", description="Calculate magnetic fields from current desity field"
    )
    parser.add_argument(
        "-i",
        "--in",
    ...
    parser.add_argument(
        "-o", "-out", dest="out", required=True, help="Output nifti file name"
    )
    parser.add_argument(
        "--res",
        dest="res",
    ...
    parser.add_argument(
        "--mask", action="store_true", help="Masks the output volume using the mesh"
    )
