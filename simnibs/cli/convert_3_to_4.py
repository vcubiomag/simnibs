...
class SubjectFiles_old:
    ...
    def __init__(self, fnamehead: str = None, subpath: str = None):
        ...
        self.eeg_cap_1010 = os.path.join(
            self.eeg_cap_folder, "EEG10-10_UI_Jurak_2007.csv"
        )
        ...
        if os.path.isdir(headreco_surf_dir):
            ...
            self.lh_midgm = os.path.join(
                headreco_surf_dir, "lh.central.T1fs_conform.gii"
            )
            self.rh_midgm = os.path.join(
                headreco_surf_dir, "rh.central.T1fs_conform.gii"
            )
            self.lh_reg = os.path.join(
                headreco_surf_dir, "lh.sphere.reg.T1fs_conform.gii"
            )
            self.rh_reg = os.path.join(
                headreco_surf_dir, "rh.sphere.reg.T1fs_conform.gii"
            )
        ...
        self.T1 = os.path.join(self.subpath, "T1fs_nu_conform.nii.gz")
...
def convert_old_new(subpath_old, subpath_new):
    ...
    if not subpath_new:
        ...
        if not pn.startswith("m2m_"):
            raise IOError(
                "The new folder name has to start with 'm2m_'. Now it is: "
                + subpath_new
            )
    ...
    if os.path.abspath(sf_old.subpath) == os.path.abspath(sf_new.subpath):
        raise FileExistsError(
            "The names of the m2m-folders for the old and converted data have to differ"
        )
        ...
        if sf_old.seg_type == "mri2mesh":
            for hemi in ["lh", "rh"]:
                wm_surface = mesh_io.read_freesurfer_surface(
                    getattr(sf_old, hemi + "_wm")
                )
                gm_surface = mesh_io.read_freesurfer_surface(
                    getattr(sf_old, hemi + "_gm")
                )
                ...
                reg_surf = mesh_io.read_freesurfer_surface(
                    getattr(sf_old, hemi + "_reg")
                )
            ...
            for hemi in ["lh", "rh"]:
                middle_surf = mesh_io.read_gifti_surface(
                    getattr(sf_old, hemi + "_midgm")
                )
                fn_out = os.path.join(sf_new.surface_folder, hemi + ".central.gii")
                mesh_io.write_gifti_surface(middle_surf, fn_out)
...
Note:
    ...
    parser = argparse.ArgumentParser(prog="convert_3_to_4", usage=usage_text)
