...
def gpc_regression_instance():
    ...
    regobj = simnibs_gpc.gPC_regression(
        [1, 2], pdftype, pdfshape, limits, poly_idx, coords_norm, "TMS"
    )
    return regobj
...
class TestgPC_Regression:
    def test_gPC_Regression_initialization(self, gpc_regression_instance):
        ...
        np.testing.assert_almost_equal(
            gpc_regression_instance.grid.coords, coords_norm * 2
        )
    ...
    def test_save_hdf5(self, gpc_regression_instance):
        ...
        with h5py.File("test.hdf5", "r") as f:
            ...
            assert np.allclose(
                f["gpc_object/limits"], np.array([[-2, -1e10], [2, 1e10]])
            )
            assert np.allclose(
                f["gpc_object/poly_idx"], np.array(np.array([[0, 0], [1, 0], [0, 1]]))
            )
            assert np.allclose(
                f["gpc_object/grid/coords_norm"], np.array([[0.5, 0.2], [0.1, 0.9]])
            )
    ...
    def test_load_hdf5(self, gpc_regression_instance):
        ...
        assert np.allclose(gpc_regression_instance.poly_idx, gpc_regression.poly_idx)
        assert np.allclose(
            gpc_regression_instance.grid.coords_norm, gpc_regression.grid.coords_norm
        )

        os.remove("test.hdf5")

    def test_postprocessing(self, sphere3_msh):
        fn = "gpc_testing.hdf5"
        ...
        gpc_reg = simnibs_gpc.gPC_regression(
            [3],
            ["beta"],
            [[1], [1]],
            [[0], [10]],
...
def sampler_args(sphere3_msh):
    ...
    return sphere3_msh, poslist, fn_hdf5, [3]
...
class TestSampler:
    ...
    def test_tdcs_run(self, mock_fem, sampler_args):
        ...
        with h5py.File(fn_hdf5, "r") as f:
            ...
            assert np.allclose(
                f["mesh_roi/data_matrices/E_samples"][0, :], [-1e3, 0.0, 0.0]
            )
            assert np.allclose(
                f["mesh_roi/data_matrices/E_samples"][1, :], [1e3, 0.0, 0.0]
            )
