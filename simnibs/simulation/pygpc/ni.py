...
def save_data_txt(data, fname):
    ...
    np.savetxt(
        fname, data, fmt="%.10e", delimiter="\t", newline="\n", header="", footer=""
    )
...
def calc_Nc(order, dim):
    ...
    return scipy.special.factorial(order + dim) / (
        scipy.special.factorial(order) * scipy.special.factorial(dim)
    )
...
def pdf_beta(x, p, q, a, b):
    ...
    return (
        (
            scipy.special.gamma(p)
            * scipy.special.gamma(q)
            / scipy.special.gamma(p + q)
            * (b - a) ** (p + q - 1)
        )
        ** (-1)
        * (x - a) ** (p - 1)
...
def run_reg_adaptive(
    ...
    while eps_gpc > eps:
        ...
        if i_iter == 1:
            ...
            grid_init = randomgrid(
                pdftype, pdfshape, limits, np.ceil(1.2 * calc_Nc(order, DIM))
            )
            regobj = reg(
                pdftype,
                pdfshape,
            ...
            if interaction_order_count == 0:
                ...
                poly_idx_all_new = poly_idx_all_new[
                    np.sum(poly_idx_all_new, axis=1) == order
                ]
            ...
            poly_idx_added = poly_idx_all_new[
                interaction_order_list == interaction_order_count, :
            ]
...
class gpc:
    ...
    def setup_polynomial_basis(self):
        ...
        for i_DIM in range(self.DIM):
            for i_order in range(Nmax + 1):
                if self.pdftype[i_DIM] == "beta":
                    p = self.pdfshape[0][
                        i_DIM
                    ]  # beta-distr: alpha=p /// jacobi-poly: alpha=q-1  !!!
                    q = self.pdfshape[1][
                        i_DIM
                    ]  # beta-distr: beta=q  /// jacobi-poly: beta=p-1   !!!
                    ...
                    jacobi_norm = (
                        2 ** (p + q - 1)
                        / (2.0 * i_order + p + q - 1)
                        * scipy.special.gamma(i_order + p)
                        * scipy.special.gamma(i_order + q)
                        / (
                            scipy.special.gamma(i_order + p + q - 1)
                    ...
                    self.poly[i_order][i_DIM] = scipy.special.jacobi(
                        i_order, q - 1, p - 1, monic=0
                ...
                if self.pdftype[i_DIM] == "normal" or self.pdftype[i_DIM] == "norm":
                    ...
                    # add entry to polynomial lookup table
                    self.poly[i_order][i_DIM] = scipy.special.hermitenorm(
                        i_order, monic=0
                    ) / np.sqrt(self.poly_norm[i_order, i_DIM])
        ...
        if self.DIM == 1:
            self.poly_idx = (
                np.array([np.linspace(0, self.order_max, self.order_max + 1)])
                .astype(int)
                .transpose()
            )
        else:
        ...
        for i_DIM in range(self.DIM):
            ...
            # delete multi-indexes from list when they exceed individual max order of parameter
                ...
                self.poly_idx = self.poly_idx[
                    self.poly_idx[:, i_DIM] <= self.order[i_DIM], :
                ]

        # Consider interaction order (filter out multi-indices exceeding it)
        if self.interaction_order < self.DIM:
            self.poly_idx = self.poly_idx[
                np.sum(self.poly_idx > 0, axis=1) <= self.interaction_order, :
            ]
        ...
        for i_poly in range(self.poly_idx.shape[0]):
            for i_DIM in range(self.DIM):
                self.poly_norm_basis[i_poly] *= self.poly_norm[
                    self.poly_idx[i_poly, i_DIM], i_DIM
                ]
    ...
    def enrich_polynomial_basis(self, poly_idx_added, form_A=True):
        ...
        for i_DIM in range(self.DIM):
            for i_order in range(order_max_current + 1, order_max_added + 1):
                if self.pdftype[i_DIM] == "beta":
                    ...
                    # add entry to polynomial lookup table
                    self.poly[i_order][i_DIM] = (
                        scipy.special.jacobi(i_order, q - 1, p - 1, monic=0)
                        / np.sqrt(self.poly_norm[i_order, i_DIM])
        ...
        for i_poly in range(N_poly_new):
            for i_DIM in range(self.DIM):
                poly_norm_basis_new[i_poly] *= self.poly_norm[
                    poly_idx_added[i_poly, i_DIM], i_DIM
                ]
        ...
        if form_A:
            ...
            for i_poly_new in range(N_poly_new):
                ...
                for i_DIM in range(self.DIM):
                    A1 *= self.poly[poly_idx_added[i_poly_new][i_DIM]][i_DIM](
    ...
    def enrich_gpc_matrix_samples(self, N_samples_N_poly_ratio):
        ...
        N_grid_new = int(
            np.ceil(N_samples_N_poly_ratio * self.A.shape[1] - self.A.shape[0])
        )

        if N_grid_new > 0:
            # Generate new grid points
            newgridpoints = randomgrid(
                self.pdftype, self.pdfshape, self.limits, N_grid_new
            )
            ...
            for i_poly in range(self.N_poly):
                ...
                for i_DIM in range(self.DIM):
                    a1 *= self.poly[self.poly_idx[i_poly][i_DIM]][i_DIM](
                        newgridpoints.coords_norm[:, i_DIM]
                    )
                a[:, i_poly] = a1
    ...
    def MC_sampling(self, coeffs, N_samples, output_idx=[]):
        ...
        for i_DIM in range(self.DIM):
            if self.pdftype[i_DIM] == "beta":
                samples_in[:, i_DIM] = (
                    np.random.beta(
                        self.pdfshape[0][i_DIM], self.pdfshape[1][i_DIM], [N_samples, 1]
                    )
                    * 2.0
                    - 1
                )[:, 0]
    ...
    def sobol(self, coeffs):
        ...
        if N_coeffs == 1:
            raise Exception(
                "Number of coefficients is 1 ... no sobol indices to calculate ..."
            )
        ...
        for i_sobol in range(N_sobol_available):
            sobol[i_sobol, :] = np.sum(
                np.square(coeffs[sobol_poly_idx[:, i_sobol] == 1, :]), axis=0
            )
    ...
    def globalsens(self, coeffs):
        ...
        for i_DIM in range(self.DIM):
            if self.pdftype[i_DIM] == "beta":  # Jacobi polynomials
                knots_list_1D[i_DIM], weights_list_1D[i_DIM] = quadrature_jacobi_1D(
                    Nmax, self.pdfshape[0][i_DIM] - 1, self.pdfshape[1][i_DIM] - 1
                )
            if (
                self.pdftype[i_DIM] == "norm" or self.pdftype[i_DIM] == "normal"
            ):  # Hermite polynomials
                knots_list_1D[i_DIM], weights_list_1D[i_DIM] = quadrature_hermite_1D(
                    Nmax
                )
        ...
        for i_DIM in range(self.DIM):
            for i_order in range(Nmax):
                ...
                poly_int[i_order][i_DIM] = np.sum(
                    np.dot(
                        self.poly[i_order][i_DIM](knots_list_1D[i_DIM]),
                        weights_list_1D[i_DIM],
                    )
                )
                poly_der_int[i_order][i_DIM] = np.sum(
                    np.dot(
                        self.poly_der[i_order][i_DIM](knots_list_1D[i_DIM]),
                        weights_list_1D[i_DIM],
                    )
                )
    ...
    def localsens(self, coeffs, xi):
        ...
        for i_DIM in range(self.DIM):
            for i_order in range(Nmax + 1):
                ...
                poly_der_xi[i_order][i_DIM] = self.poly_der[i_order][i_DIM](
                    xi[1, i_DIM]
                )
    ...
    def pdf(self, coeffs, N_samples, output_idx=[]):
        ...
        for i_out in range(coeffs.shape[1]):
            kde = scipy.stats.gaussian_kde(
                samples_out.transpose(),
                bw_method=0.1 / samples_out[:, i_out].std(ddof=1),
            )
            pdf_x[:, i_out] = np.linspace(
                samples_out[:, i_out].min(), samples_out[:, i_out].max(), 100
            )
...
class reg(gpc):
    def __init__(
        self, pdftype, pdfshape, limits, order, order_max, interaction_order, grid
    ):
        """
    ...
    def LOOCV(self, data):
        ...
        for i in range(data.shape[0]):
            ...
            self.relerror[i] = np.linalg.norm(
                data[i, :] - np.dot(self.A[i, :], coeffs_LOO)
...
class quad(gpc):
    ...
    def expand(self, data):
        ...
        if not (grid_pdf_fit):
            ...
            for i_DIM in range(self.DIM):
                if self.pdftype[i_DIM] == "beta":
                    joint_pdf[:, i_DIM] = pdf_beta(
                        self.grid.coords_norm[:, i_DIM],
                        self.pdfshape[0][i_DIM],
                        self.pdfshape[1][i_DIM],
                        -1,
                        1,
                    )
                if self.pdftype[i_DIM] == "norm" or self.pdftype[i_DIM] == "normal":
                    joint_pdf[:, i_DIM] = scipy.stats.norm.pdf(
                        self.grid.coords_norm[:, i_DIM]
                    )
