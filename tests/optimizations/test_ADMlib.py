...
def test_computeHprimary():
    ...
    fmm_Hprimary = ADMlib.computeHprimary(
        current_elm_positions.T, currents.T, observation_pos.T, eps
    ).T
...
def test_resample_coil_rotate():
    ...
    resampled_positions, resampled_weights = ADMlib.resamplecoil(
        coil_dipole_pos.T, coil_dipole_weights.T, [17, 17, 2], len(coil_dir), coil_dir.T
    )
...
def test_ADM_no_rotate():
    ...
    for i, p in enumerate(observation_pos):
        H = calculate_Hprim(current_elm_positions, currents, coil_dipole_pos + p)
...
def test_ADM_rotate():
    ...
    for i, p in enumerate(observation_pos):
        for j, cd in enumerate(coil_dir):
            ...
            H = calculate_Hprim(current_elm_positions, currents, coil_dipole_pos_moved)
