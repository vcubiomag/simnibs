...
class Ui_Electrode(QtWidgets.QDialog):
    def __init__(self, electrode_struct):
        ...
        self.glElectrode.electrode_object, self.glElectrode.plug_object = (
            self.glElectrode.drawElectrode(self.electrode_struct)
        )
    ...
    def set_type_tick_layout(self):
        ...
        if (
            ...
            or len(self.electrode_struct.thickness) == 3
        ):
        ...
        if (
            ...
            and self.electrode_struct.dimensions_sponge is not None
        ):
            self.spo_x.setValue(self.electrode_struct.dimensions_sponge[0] / 10.0)
            self.spo_y.setValue(self.electrode_struct.dimensions_sponge[1] / 10.0)
    ...
    def set_connector_layout(self):
        ...
        self.label_con_pos = QtWidgets.QLabel(
            "Position in relation to electrode center:"
        )
    ...
    def update_electrode(self):
        ...
        # write dimensions
        self.electrode_struct.dimensions = [
            self.dim_x.value() * 10,
            self.dim_y.value() * 10,
        ]
        ...
        if self.type_selector.currentIndex() == 2:
            ...
            self.electrode_struct.dimensions_sponge = [
                self.spo_x.value() * 10,
                self.spo_y.value() * 10,
            ]
            self.spo_x.setMinimum(1.0 * self.dim_x.value())
            self.spo_y.setMinimum(1.0 * self.dim_y.value())
            self.dim_x.setMaximum(1.0 / 1.0 * self.spo_x.value())
            self.dim_y.setMaximum(1.0 / 1.0 * self.spo_y.value())
        ...
        if self.Yes_Connector.isChecked():
            ...
            self.electrode_struct.plug[0].dimensions = [
                self.con_dim_x.value() * 10,
    ...
    def update_labels(self):
        if self.type_selector.currentIndex() == 0:
            ...
            if (
                self.electrode_struct.thickness is not None
                and len(self.electrode_struct.thickness) == 1
            ):
                self.thick1.setValue(self.electrode_struct.thickness[0])
            else:
        ...
        if self.type_selector.currentIndex() == 1:
            ...
            if (
                self.electrode_struct.thickness is not None
                and len(self.electrode_struct.thickness) == 2
            ):
                self.thick2.setValue(self.electrode_struct.thickness[0])
                self.thick1.setValue(self.electrode_struct.thickness[1])
            else:
        ...
        if self.type_selector.currentIndex() == 2:
            ...
            if (
                self.electrode_struct.thickness is not None
                and len(self.electrode_struct.thickness) == 3
            ):
...
class GLElectrode(QtWidgets.QOpenGLWidget):
    ...
    def initializeGL(self):
        self.setClearColor(QtGui.QColor.fromCmykF(0.1, 0.0, 0.2, 1.0))
        self.electrode_object, self.plug_object = self.drawElectrode(
            self.electrode_struct
        )
    ...
    def drawElectrode(self, electrode_struct):
        ...
        if len(electrode_struct.thickness) == 3:
            thick1 = electrode_struct.thickness[1] * scale
            thick2 = (
                electrode_struct.thickness[0] * scale
                + electrode_struct.thickness[1] * scale
                + electrode_struct.thickness[2] * scale
            )
            model = "sponge"
        ...
        if electrode_struct.shape == "rect":
            ...
            if model == "2layer" and thick2 > 0:
                ...
                self.cuboid(
                    0, 0, (thick1 + thick2) / 2, size_x, size_y, thick2, blue, False
                )
            if model == "sponge":
                self.cuboid(0, 0, 0, size_x, size_y, thick1, grey, False)
        ...
        if electrode_struct.shape == "ellipse":
            ...
            if model == "sponge":
                ...
                sponge_size_y = electrode_struct.dimensions_sponge[1] * scale
                self.cylinder(
                    0, 0, 0, sponge_size_x, sponge_size_y, thick2, yellow, True
                )
    ...
    def cuboid(
        self, center_x, center_y, center_z, size_x, size_y, size_z, color, wireframe
    ):
        if wireframe:
    ...
    def cylinder(
        self, center_x, center_y, center_z, x_axis, y_axis, size_z, color, wireframe
    ):
    ...
    def drawPointAndDirs(self):
        ...
        self.qglColor(QtGui.QColor.fromCmykF(0.0, 1.0, 1.0, 0.0))
        ...
        self.qglColor(QtGui.QColor.fromCmykF(1.0, 0.0, 1.0, 0.0))
