...
def _remove_unconnected_triangles(mesh, roi_triangles, center, roi_nodes, triangles):
    ...
    if len(cc) > 1:
        _, center_index = mesh.find_closest_element(
            center, return_index=True, elements_of_interest=roi_triangles
        )
        for comps in cc:
            if center_index in comps:
                roi_triangles = comps

        roi_nodes, triangles = np.unique(
            mesh.elm[roi_triangles, :3], return_inverse=True
        )
...
def _get_transform(
    ...
    y_axis,
    mesh,
    mesh_surface=[ElementTags.SCALP, ElementTags.SCALP_TH_SURFACE],
    y_type="relative",
    nodes_roi=None,
):
    ...
    if nodes_roi is None:
        ...
        normal = np.nanmean(mesh.nodes_normals().value[nodes_roi - 1], axis=0)
    ...
    if y_type == "relative":
        ...
        while np.linalg.norm(c - y_ref) < 1e-5:
            ...
            if nr_iter == 10:
                raise ValueError(
                    "Could not define coordinate system: "
        ...
        if np.isclose(np.linalg.norm(y_axis), 0):
            raise ValueError(
                "Could not define coordinate system: Y axis perpendicular to surface?"
            )
        y_axis /= np.linalg.norm(y_axis)
...
def _get_roi(center, radius, mesh, mesh_surface=[5, 1005], min_cos=0.1):
    ...
    avg_l = np.average(
        np.linalg.norm(
            mesh.nodes[mesh.elm[mesh.elm.triangles, 0][nodes_in_surface]]
            - mesh.nodes[mesh.elm[mesh.elm.triangles, 1][nodes_in_surface]],
            axis=1,
        )
    )

    nodes_in_roi_ext = nodes_in_surface[
        (distances <= radius + 2 * avg_l) * (center_normal.dot(normals.T) > min_cos)
    ]
    nodes_in_roi = nodes_in_surface[
        (distances <= radius) * (center_normal.dot(normals.T) > min_cos)
    ]
    ...
    roi_tr_nodes, roi_triangles_reordering = np.unique(
        mesh.elm[mesh.elm.triangles[tr_in_roi], :3], return_inverse=True
    )

    roi_th_nodes, roi_tetrahedra_reordering = np.unique(
        mesh.elm[mesh.elm.tetrahedra[th_in_roi]], return_inverse=True
    )

    return (
        mesh.elm.triangles[tr_in_roi],
        roi_tr_nodes,
        roi_triangles_reordering.reshape(-1, 3),
        mesh.elm.tetrahedra[th_in_roi],
        roi_th_nodes,
        roi_tetrahedra_reordering.reshape(-1, 4),
    )
...
def _point_inside_polygon(vertices, points, tol=1e-3):
    ...
    s_x = np.maximum(np.max(vertices[:, 0]), np.max(points[:, 0])) - np.minimum(
        np.min(vertices[:, 0]), np.min(points[:, 0])
    )
    s_y = np.maximum(np.max(vertices[:, 1]), np.max(points[:, 1])) - np.minimum(
        np.min(vertices[:, 1]), np.min(points[:, 1])
    )
    r = 1.5 + np.random.rand(2)
    p = [r[0] * s_x, r[1] * s_y]
    for v in range(len(vertices) - 1):
        intersection_points, inside = _line_line_intersect(
            vertices[v], vertices[v + 1], points.T, (points + p).T
        )
...
def _line_line_intersect(
    end_point_a1, end_point_a2, end_point_b1, end_point_b2, tol=1e-5
):
...
def _point_line_distance(end_point_1, end_point_2, points, tol=1e-5):
    ...
    normal = np.random.rand(len(end_point_1)) - 0.5
...
def _triangle_with_points(
    points, triangles, nodes, eps=1e-5, edge_list=None, kdtree=None
):
...
def _move_point(
    ...
    tr = _triangle_with_points(
        np.atleast_2d(new_position),
        triangles,
        tr_nodes[:, :2],
        edge_list=edge_list,
        kdtree=kdtree,
...
def _make_line(
    ...
    if ends:
        ...
        tr_w_points = _triangle_with_points(
            line, triangles, tr_nodes[:, :2], edge_list=edge_list
        )
        for p, t in zip(line, tr_w_points):
            ...
            if t != -1:
                ...
                if moved_tr_nodes_list != []:
                    ...
                    moved_th_nodes[np.where(np.isin(roi_th_nodes, roi_tr_nodes))[0]] = (
                        _apply_affine(inv_affine, moved_tr_nodes_list[gammas.argmin()])
                    )
    ...
    edge_s = side[edges]
    edges_crossing = np.where(np.prod(edge_s, axis=1) < -0.1)[0]
...
def _draw_polygon_2D(
    ...
    for v in range(len(poly)):
        ...
        moved_tr_nodes, moved_th_nodes, m = _make_line(
            vertices,
            moved_tr_nodes,
            moved_th_nodes,
            roi_tr_nodes,
...
def _inside_complex_polygon(poly, nodes, triangles, holes=[], tol=1e-2):
    ...
    if len(holes) > 0:
        tr_hole = []
        for h in holes:
            tr_hole.append(np.where(_point_inside_polygon(h, bar, tol=tol * avg_l))[0])
...
def _build_electrode(
    ...
    if middle_layer is not None and len(h) < 3:
        raise ValueError("Can only define a middle layer with sandwich electrodes")
    ...
    if not test_gamma:
        ...
        tr_inside = _inside_complex_polygon(
            poly, nodes, triangles, holes=holes, tol=1e-2
        )
        if len(tr_inside) == 0:
            return None
    ...
    for n, n_l in enumerate(n_th_layers):
        for nn in range(n_l):
            ...
            tetrahedra.append(
                np.vstack(
                    [
                        l * n_in_layer + tr_reordered[c1, 0],
            ...
            tetrahedra.append(
                np.vstack(
                    [
                        ...
                        l * n_in_layer + tr_reordered[c1, 1],
                        (l + 1) * n_in_layer + tr_reordered[c1, 2],
                        (l + 1) * n_in_layer + tr_reordered[c1, 1],
                    ]
                ).T
            )
            ...
            tetrahedra.append(
                np.vstack(
                    [
                        l * n_in_layer + tr_reordered[c2, 0],
            ...
            tetrahedra.append(
                np.vstack(
                    [
                        ...
                        (l + 1) * n_in_layer + tr_reordered[c2, 1],
                        l * n_in_layer + tr_reordered[c2, 2],
                        (l + 1) * n_in_layer + tr_reordered[c2, 2],
                    ]
                ).T
            )
            count += 1
    ...
    if len(h) == 1 or len(h) == 2:
        ...
        if middle_layer is None:
            middle_layer = poly
        tr_mid = _inside_complex_polygon(
            middle_layer, nodes, triangles, holes=holes, tol=1e-2
        )
    ...
    if plug is not None:
        ...
        triangles_idx = np.hstack((triangles_idx, -np.ones(np.sum(plug_tr), dtype=int)))
...
def _build_electrode_on_mesh(
    ...
    el_vol_tag,
    el_surf_tag,
    on_top_of=[ElementTags.SCALP, ElementTags.SCALP_TH_SURFACE],
    holes=[],
    plug=None,
    plug_tag=None,
    middle_layer=None,
    ...
    roi_tr_nodes, triangles, roi_triangles = _remove_unconnected_triangles(
        mesh, roi_triangles, center, roi_tr_nodes, triangles
    )

    tr_nodes = mesh.nodes[roi_tr_nodes]
    affine, _ = _get_transform(
        center, ydir, mesh, mesh_surface=on_top_of, nodes_roi=roi_tr_nodes
    )
    ...
    mesh.nodes.node_coord = np.vstack([mesh.nodes.node_coord, moved_tr_nodes])
    ...
    node_dict[~in_surf] = (
        1 + mesh.nodes.nr - len(moved_tr_nodes) + np.arange(len(moved_tr_nodes))
    )
    ...
    # Make the tetrahedra
    mesh.elm.tag1 = np.hstack([mesh.elm.tag1, tetra_tags])
    mesh.elm.tag2 = np.hstack([mesh.elm.tag2, tetra_tags])
    ...
    mesh.elm.tag1 = np.insert(mesh.elm.tag1, s, surf_tags)

    mesh.elm.tag2 = np.insert(mesh.elm.tag2, s, surf_tags)

    mesh.elm.elm_type = np.insert(
        mesh.elm.elm_type, s, int(2) * np.ones(len(tr), dtype=int)
    )

    mesh.elm.node_number_list = np.insert(mesh.elm.node_number_list, s, tr, axis=0)

    return mesh


def _create_polygon_from_elec(
    elec, mesh, skin_tag=[ElementTags.SCALP, ElementTags.SCALP_TH_SURFACE]
):
    if elec.definition == "plane":
        if elec.shape in ["rect", "rectangle", "ellipse"]:
            ...
            try:
                x = float(elec.dimensions[0]) / 2.0
                y = float(elec.dimensions[1]) / 2.0
            except IndexError:
                x = float(elec.dimensions[0]) / 2.0
                y = float(elec.dimensions[0]) / 2.0
            except TypeError:
                x = float(elec.dimensions) / 2.0
                y = float(elec.dimensions) / 2.0
        ...
        if elec.shape in ["rect", "rectangle"]:
            ...
            assert poly.shape[1] == 2, (
                "Vertices in electrode space must have 2 dimensions"
            )
        else:
            raise ValueError(
                'Invalid electrode shape: {0}, valid shapes are "rect",'
        ...
        if len(center) == 2:  # If the center is 2-dimensional, it is relative
            ...
            raise ValueError(
                "Wrong dimension of electrode centre: it should be 1x3 (or 1x2 for plugs)"
            )
        ...
        _, roi_nodes, _, _, _, _ = _get_roi(center, R, mesh, mesh_surface=skin_tag)
        transform, c = _get_transform(
            center, None, mesh, mesh_surface=skin_tag, nodes_roi=roi_nodes
        )
        center = c
        y_axis = None
...
def put_electrode_on_mesh(
    elec,
    mesh,
    elec_tag,
    skin_tag=[ElementTags.SCALP, ElementTags.SCALP_TH_SURFACE],
    extra_add=(ElementTags.SALINE_START - ElementTags.ELECTRODE_RUBBER_START),
    surf_add=ElementTags.TH_SURFACE_START,
    plug_add=ElementTags.ELECTRODE_PLUG_SURFACE_START,
):
    ...
    elec_poly, elec_center, ydir = _create_polygon_from_elec(
        elec, mesh, skin_tag=skin_tag
    )
    thick = np.atleast_1d(elec.thickness)
    holes_poly = []
    for h in elec.holes:
        holes_poly.append(_create_polygon_from_elec(h, mesh, skin_tag=skin_tag)[0])
    ...
    if elec.plug in [[], None]:
        ...
        try:
            ...
            plug = elec.plug
        plug_poly, plug_center, plug_ydir = _create_polygon_from_elec(
            plug, mesh, skin_tag=skin_tag
        )

    if elec.dimensions_sponge is not None and len(elec.dimensions_sponge) > 0:
        if elec.definition != "plane":
            raise NotImplementedError(
                "Sponges are only implemented for plane electrodes"
            )
        if elec.shape not in ["rect", "ellipse"]:
            raise NotImplementedError(
                "Sponges are only implemented for rectangular and elliptical electrodes"
            )
    ...
    roi_triangles, roi_tr_nodes, triangles, roi_tetrahedra, roi_th_nodes, tetrahedra = (
        _get_roi(elec_center, R, mesh, mesh_surface=skin_tag)
    )

    # Sometimes the ROI can include nodes inside the head. Figure out the
    # connected components and take the group, which includes the center node
    roi_tr_nodes, triangles, roi_triangles = _remove_unconnected_triangles(
        mesh, roi_triangles, elec_center, roi_tr_nodes, triangles
    )
    ...
    affine, elec_center = _get_transform(
        elec_center, ydir, mesh, mesh_surface=skin_tag, nodes_roi=roi_tr_nodes
    )
    tr_nodes = _apply_affine(affine, tr_nodes)
